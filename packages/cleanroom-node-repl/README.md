...
The clean-room server is CodeAct-style: the model writes JavaScript and manipulates persistent state and external context inside the REPL. After conditional bootstrap, the stable `linkedScience` object provides composable broker-mediated `resources`, native `rdf`, and Communica `traversal` APIs. This follows the useful Browser/Playwright architecture pattern—a persistent JavaScript object model above a trusted broker—without depending on OpenAI-private internals. `nodeRepl.rlm` supplies context registration, slicing, and an optional broker-mediated recursive-call seam; recursion is unavailable by default. `nodeRepl.peek` supplies a context-scoped, bounded PEEK-compatible orientation map. The token/epoch-bound traversal bridge and ambient Fetch closure are module-private: neither appears on `nodeRepl` or the REPL global.
