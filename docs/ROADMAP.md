...
| Slice | Hypothesis | Acceptance evidence | Status |
| --- | --- | --- | --- |
| Blind clean-worker map evaluation | A fresh worker can use repository-local map/session guidance rather than prior conversation or raw rows. | Fresh-task trace proves persistent-REPL preflight before any satisfiable source analysis, an execution-backed conclusion, map-first orientation, cited bounded handle reuse/derivation/display, provenance/budget reporting, and no external access. | Partially validated — one constrained local, coordinator-steered run passed on 2026-08-15; prior open runs remain failed/inconclusive preflights. Open-ended navigation and external data remain untested. See [protocol](experiments/clean-worker-map-evaluation.md). |
| Schema-derived affordance catalog | A worker can use ontology/documentation-derived prefix and motif affordances to plan a bounded query before endpoint exploration. | Catalog lookup/plan/validation tests; clean-worker evaluation requires a catalog-plan receipt before construction and compares trajectory against a no-catalog control. | Implemented locally — no clean-worker or live federation evaluation yet. See [protocol](experiments/affordance-catalog.md). |
