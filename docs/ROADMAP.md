...
| Slice | Hypothesis | Acceptance evidence | Status |
| --- | --- | --- | --- |
| Blind clean-worker map evaluation | A fresh worker can use repository-local map/session guidance rather than prior conversation or raw rows. | Fresh-task trace proves persistent-REPL preflight, map-first orientation, cited bounded handle reuse/derivation/display, provenance/budget reporting, and no external access. | Planned — the prior open clean run was an informative failed preflight, not a pass; no qualifying evaluation has run. See [protocol](experiments/clean-worker-map-evaluation.md). |
