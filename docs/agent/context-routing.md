...
| Maintain saved scientific workflow patterns | [Scientific wiki](../../wiki/README.md), then the [maintainer skill](../../.agents/skills/scientific-wiki-maintainer/SKILL.md). Use explicit eligible corpus records and preserve proposed/reviewed status. |
| Integrate reasoning or learn when to use it | [Reasoning and wiki handoff](../tasks/reasoning-and-wiki-guidance.md), then its compatibility and session references. Preserve the order: working surface → scientific episodes → candidate guidance → transfer evaluation. |
| Plan wiki memory or scientific skill evolution | [Wiki-memory implementation plan](../tasks/wiki-memory-continual-learning.md); distinguish host history, built-in memory and project evidence before adding capture or runtime retrieval. |
