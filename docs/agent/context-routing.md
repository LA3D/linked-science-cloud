...
| Design or run the UniProt competency evaluation | The [competency-question methodology](../experiments/uniprot-competency-question-evaluation.md), then the active [manifest task](../tasks/uniprot-competency-manifest.md). Keep official queries evaluator-only; each live profile still needs current exact approval. |
