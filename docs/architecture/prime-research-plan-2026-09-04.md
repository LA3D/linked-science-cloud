> Historical design record captured before the scientific REPL simplification on 2026-09-04. Its scope and authorizations are historical. The [current plan](../../PLAN.md) makes this program an optional research direction. No durable/provider/policy implementation follows automatically from this record.

# Prime-inspired durable RLM, context, and continual-harness research plan

**Status at capture:** Active evidence-gated implementation plan. Phase 0, symbolic-graph realignment, query completeness, out-of-core graph-result retention, and the heap-aligned residency/indexed-spool correction were completed locally on 2026-09-04. Recursive-provider, PEEK-policy, and durable Prime work remain gated.
