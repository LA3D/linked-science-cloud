...
The MCP surface remains exactly `js`, `js_reset`, and `js_add_node_module_dir`. The Linked Science workspace exposes direct evidence retention, mediated query, result inspection, derivation, and orientation operations. The runtime does not choose discovery locations, endpoints, ontology meanings, or query structure. Consumer-owned Communica executes each direct bounded query and returns handles retaining native RDF/JS bindings, quad streams materialized within bounds, or booleans. No document-fetch MCP tool, `workspace.documents` method, plan-enrollment protocol, or parallel graph/dataset model exists.
