...
The private lifecycle remains `beginTraversal`, mediated requests, receipt snapshots, and `finishTraversal`/`abortTraversal`. Grounding discovery and scored traversal use distinct bounded sessions. The agent completes discovery, evidence attestation, and immutable planning before `traversal.begin({ plans, budgets })` starts the scored cumulative wall clock. Only enrolled plans may execute, and each is consumed once. Requests, distinct sources, bytes, elapsed time, and retained result items for those plans consume one cumulative scored budget scope. The traversal-wide wall-clock default is five minutes with a fifteen-minute hard ceiling, while each request remains independently capped at twenty seconds by default and thirty seconds at most. Sessions bind to the current kernel capability token and epoch and expire on finish, abort, duration, reset, crash, or child replacement. This thin phase boundary is evidence/accounting state attached to the Codex goal, not a competing task state machine.
