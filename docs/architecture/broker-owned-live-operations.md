...
The MCP surface remains exactly `js`, `js_reset`, and `js_add_node_module_dir`. The Linked Science workspace keeps one generic grounding boundary in front of `workspace.traversal.query`. Grounding accepts declarative documents and resident typed handles, permits optional bounded discovery through the same mediator, attests evidence-backed source/graph/predicate choices, freezes immutable plans, and registers compact evidence and plan metadata in an RLM context. The runtime does not choose discovery locations, endpoints, ontology meanings, or query structure. Consumer-owned Communica executes explicitly enrolled plans and returns handles retaining native RDF/JS bindings, quad streams materialized within bounds, or booleans. No document-fetch MCP tool, `workspace.documents` method, or parallel graph/dataset model exists.
...
The private lifecycle remains `beginTraversal`, mediated requests, receipt snapshots, and `finishTraversal`/`abortTraversal`. Grounding discovery and scientific traversal use distinct bounded sessions. The agent completes discovery, evidence attestation, and an immutable initial plan before `traversal.begin({ plans, budgets, iterationPolicy })` starts the cumulative wall clock. Only enrolled plans may execute; later immutable revisions require an explicit enrollment receipt. Explicit query attempts may iterate up to `maxScientificQueries`, while every request, distinct source, byte, elapsed millisecond, and retained item remains in the same cumulative scope. The transport itself performs no hidden retries. An evaluation can freeze a one-attempt policy without turning that measurement choice into a generic runtime invariant. Sessions bind to the current kernel capability token and epoch and expire on finish, abort, duration, reset, crash, or child replacement. This thin phase boundary is evidence/accounting state attached to the Codex goal, not a competing task state machine.

## Compositional grounding boundary

The design applies the principle described by Alex Zhang and Omar Khattab in “Language model harnesses are compositional generalizers”: different domain tasks should become locally in-distribution and structurally isomorphic by offloading domain information while preserving a generic control trajectory. Every resource therefore uses the same boundary—orient or discover, attest typed evidence, freeze a plan, then execute or inspect. Resource-specific schemas, vocabularies, endpoints, identifiers, graphs, and predicates remain declarative evidence or REPL state. A grounded typed result can be selected as evidence for a later resource through the same handle contract, enabling multi-hop composition without special-case helpers.

This is deliberately not a workflow engine. The runtime enforces only phase ordering, resident evidence provenance, evidence-backed declared choices, immutable plan identity, context registration, and scored-timer ordering. Discovery strategy, standard-location search order, endpoint selection, ontology interpretation, query construction, scientific reasoning, and stopping decisions remain with the agent under concise skill guidance.
...
Retrieved bytes are untrusted data. They never become instructions, raw model tokens, PEEK orientation, skills, wiki memory, or evaluator state automatically. Successful Communica operations retain native RDF/JS values behind epoch-bound handles. Only bounded attestation metadata and immutable plan hashes enter the registered grounding RLM context; payloads do not. Bounded profile/page/neighborhood projections are explicit subsequent operations. A receipt proves transport and result lineage, not a scientific interpretation.
