...
The lifecycle remains `beginTraversal`, mediated request, and `finishTraversal`/`abortTraversal`. Sessions are immutable, accept only budgets at or below hard ceilings, bind to the current kernel capability token and epoch, and expire on finish, abort, duration, reset, crash, or child replacement. Direct agent imports of Communica receive no injected Fetch; Node permission enforcement supplies no network authority.
