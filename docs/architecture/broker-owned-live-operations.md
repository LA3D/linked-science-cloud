...
The MCP surface remains exactly `js`, `js_reset`, and `js_add_node_module_dir`. The Linked Science workspace exposes direct evidence retention, general broker-mediated resources, RDF/JS datasets, mediated query, result inspection, derivation, and orientation operations. The runtime does not choose discovery locations, endpoints, ontology meanings, or query structure. `workspace.resources.get(url)` returns a retained response-like object for anonymous `GET`/`HEAD`, with in-kernel JSON/text/binary methods and bounded representation-aware inspection. Consumer-owned Communica executes each direct bounded query and returns handles retaining native RDF/JS bindings, quad streams materialized within bounds, or booleans. There is no additional document-fetch MCP tool or plan-enrollment protocol.

An RDF document, ontology, service description, or VoID graph can be acquired as a general resource and parsed directly: `const resource = await workspace.resources.get(url); const graph = await resource.rdf({ name: 'source-graph' });`. Its bytes, media type, exchange receipt, aggregate receipt, and source handle are retained automatically. `workspace.rdf.dataset(graph)` returns a clone suitable for JavaScript composition, and `workspace.rdf.retain({ dataset })` retains modified RDF for a local Communica query without re-fetching. Typed SPARQL sources, heterogeneous sources, and `SERVICE` remain on `workspace.traversal.query`. Complete means complete within the effective byte and quad/item ceilings; exceeding either is an attributable bounded failure.
