...
The [runtime API schema](linked-science-api.schema.json) is the machine-readable surface for the clean-room MCP and explicit roots, bootstrap, stable bindings, discovery, workspace groups, broker-owned PEEK orientation, optional broker-owned live operations, and local compatibility adapters. The [broker capability schema](linked-science-broker-capability.schema.json) defines the serializable named-profile and receipt boundary. The [route index](routes.json) lists exact conditional-documentation names and lookup aliases.
