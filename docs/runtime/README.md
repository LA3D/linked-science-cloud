...
The [runtime API schema](linked-science-api.schema.json) is the machine-readable surface for the authoritative project identity, repository-owned clean-room MCP, explicit roots, bootstrap, stable bindings, discovery, general broker-mediated resource responses, RDF/JS composition, broker-owned PEEK orientation, mediated Communica traversal, and local compatibility adapters. The [traversal capability schema](linked-science-traversal-capability.schema.json) defines the anonymous-read authority plus aggregate/per-exchange receipt boundary. The [route index](routes.json) lists exact conditional-documentation names and lookup aliases.
