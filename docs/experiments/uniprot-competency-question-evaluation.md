...
4. Give the worker the natural question, worker-visible opaque case ID, bounds, and authorization only. Do not provide the official query, example number, expected answer, predicates, graph path, operation, handle name, or prior trace.
5. Require chronological clean-room tool receipts, guarded transport receipts, retained handles, bounded profiles/pages/tables, compact provenance, and calibrated uncertainty. Bulk documents and results remain resident.
