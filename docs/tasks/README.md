...
| --- | --- | --- | --- |
| [UniProt competency evaluation manifest](uniprot-competency-manifest.md) | Standard-Fetch replacement verified offline; restart/preflight pending; baseline gate closed | Preserve the leakage-audited three-case public/private split while workers use only generic mediated traversal budgets and resource roles. | Restart Desktop, verify protocol 3.0.0 activation, then obtain separate authorization for one new neutral preflight before any baseline. |
