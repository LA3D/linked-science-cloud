...
| [Reasoning integration and wiki guidance](reasoning-and-wiki-guidance.md) | First runtime slice implemented; live activation next | Connect deterministic reasoning to the shared symbolic surface, then learn when to use it from scientific episodes. | Activate runtime 6.5.0 in a fresh broker/service, then record a scientific worker episode before proposing wiki guidance. Automated real-engine scoped tests pass; selected-conclusion explanation and guidance evaluation remain. |
...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Scientific corpus and phase-2 wiki maintenance implemented | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | See the [phase-2 handoff](scientific-wiki-maintainer-handoff.md). The [independent evidence review](wiki-independent-recovery-review.md) is inconclusive; two seed patterns remain proposed. [Selective retrieval](wiki-selective-retrieval.md) and a [fresh-context pilot](wiki-live-comparison.md) pass functional checks; both arms recovered correctly, so incremental benefit remains unestablished. No automatic skill release. |
