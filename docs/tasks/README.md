...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Phase-0 foundation and bounded phase-1 import implemented | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | Review the selected real episode and coverage limits; authorize a selected wiki-maintenance batch before phase 2. No automatic archive, hooks or memory writes. |
