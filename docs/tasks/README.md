...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Scientific-only eligibility and one observed workflow implemented | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | Review eligible scientific candidates and their limits before authorizing maintenance; exclude engineering/planning fixtures. No automatic learning, hooks or memory writes. |
