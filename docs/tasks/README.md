...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Implementation plan ready; execution not authorized | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | Review the plan; after authorization, begin phase 0 contracts and bounded history/receipt coverage. Preserve frozen baselines; no automatic archive, hooks or memory writes. |
