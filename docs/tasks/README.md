...
| --- | --- | --- | --- |
| [UniProt competency evaluation manifest](uniprot-competency-manifest.md) | Neutral live preflight failed; baseline gate closed | Preserve the leakage-audited three-case public/private split while workers use only generic mediated traversal budgets and resource roles. | Diagnose the DNS-pinned HTTPS handoff offline; after a fix, obtain separate authorization for one new neutral preflight before any baseline. |
