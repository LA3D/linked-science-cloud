...
| --- | --- | --- | --- |
| [Prime-style Linked Data context management](../architecture/prime-linked-data-context-management.md) | Design ready; provider/policy implementation not authorized | Determine whether depth-one handle reasoning and a real PEEK policy improve dense Linked Data work before building durable Prime machinery. | Freeze two Gate A synthetic fixtures, matched budgets, scoring, typed-event requirements, and stop/go thresholds; select a provider only under separate authorization. |
