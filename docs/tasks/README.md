...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Scientific corpus and phase-2 wiki maintenance implemented | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | See the [phase-2 handoff](scientific-wiki-maintainer-handoff.md). The [independent evidence review](wiki-independent-recovery-review.md) is inconclusive; two seed patterns remain proposed. [Selective retrieval](wiki-selective-retrieval.md) is implemented; independent scientific recovery and transfer benefit remain unmeasured. No automatic skill release. |
