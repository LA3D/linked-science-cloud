...
| [Wiki memory and reviewed skill evolution](wiki-memory-continual-learning.md) | Phase-0 foundation implemented; later phases proposed | Maintain an independent repository-owned Science Cloud wiki, separate from Obsidian wikimemory; reuse Codex history/receipts and validate atomic skill releases with discovery and rollback checks. | Verify the saved contracts/fixture; before phase 1 select and authorize bounded host input and retention. No automatic archive, hooks or memory writes. |
