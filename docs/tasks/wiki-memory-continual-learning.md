...
**Next implementation boundary:** engineering foundations, explicit corpus eligibility and one genuine scientific workflow episode are complete. Before wiki maintenance, authorize a batch of eligible scientific units and review the proposed hypothesis scope; engineering fixtures must stay excluded. Candidate eligibility does not authorize automatic learning or validate procedural transfer. Do not install hooks or design a new archive first. No additional decision is needed to save/review this plan. Before a real pilot, material choices are (a) which selected task evidence the user permits retaining beyond references, (b) a supported host connection or manual import mode, and (c) a genuinely isolated evaluator/memory treatment. Defaults above allow synthetic contract work while these are resolved.
...
## 14. Scientific-workflow scope correction and first eligible episode

The user clarified that wiki memory is for **scientific RLM/REPL workflows, including retrieval and REPL operations**, not software development. The original phase-1 planning import is now explicitly classified as engineering test evidence by a [versioned exclusion overlay](../../artifacts/wiki-learning/corpus/planning-engineering.json), backed by the [exclusion ledger](../../artifacts/wiki-learning/corpus/exclusions.json). Its original episode and observations were not edited. Obsidian separation remains an architectural constraint, not a scientific procedural lesson. Earlier sections describe historical engineering work; their existence does not make those fixtures eligible learning inputs.

The [corpus schema](../../schemas/wiki-learning/corpus.schema.json) and [filter](../../lib/wiki-learning/corpus.mjs) use a reviewed scientific objective → actual retrieval/REPL actions → observed outcome as the unit. They admit success and failure candidates, explicitly exclude engineering objectives in mixed records, verify source/entity anchors against actual actions, and compare normalized observations and query profiles with hashed raw broker exchanges. Relabeling the planning fixture cannot admit it. Files with no explicit eligibility record are not scanned into the corpus. Scope review here is curator judgment under the user's stated scope, not automated semantic truth or an independent human approval. No schema can prove that arbitrary natural-language interpretation is scientifically correct.

Episode **1.1.0** adds `direct-runtime` capture: an objective may be declared in an actual project tool output, and event IDs belong to the recorded broker run. This avoids inventing user messages or Codex task IDs for programmatic scientific actions. The existing 1.0.0 episode/import contracts remain readable and their tests pass. The v1 corpus witness adapter supports the run's project-stdio exchange format only; other host formats need a validated adapter. It does not treat summary-only or inferred history joins as direct runtime evidence.

The [new scientific episode](../../artifacts/wiki-learning/scientific/ontology-membership-20260919/episode.json) compares asserted GO memberships in human hemoglobin alpha/beta UniProt records. The source-owned relation definition, broker-mediated retrievals, graph profiles, complete query lineage and independent native-set check are recorded. Counts: alpha 20, beta 25, intersection 18, union 27; the [membership matrix](../../artifacts/wiki-learning/scientific/ontology-membership-20260919/membership.svg) displays all 45 selected protein–GO edges. This is annotation overlap, not ontology hierarchy, functional equivalence or a pathway reconstruction. Protein payload hashes/time identify the source snapshots; the schema ETag is not assumed to establish the protein release.

A real projection failure (`limit:100`, maximum 10) and recovery using bounded paging/native iteration are part of that scientific workflow. The query stayed retained; its complete answer was independently checked. The [eligibility record](../../artifacts/wiki-learning/corpus/ontology-membership.json) keeps the source-dependent finding separate from two procedural candidates (observed failure and recovery). Each carries scope, source evidence, freshness/revalidation semantics and `candidate-not-generalized`. Incomplete eligible units remain `incomplete` and are excluded from the ready-candidate list. No actual pattern extraction, maintainer model, skill evolution, release or transfer evaluation has occurred.

The [experiment dossier](../experiments/scientific-workflow-corpus.md) distinguishes the failed mounted-session probe from the fresh actual broker connection used successfully. The [receipt](../../artifacts/wiki-learning/scientific/ontology-membership-20260919/receipt.json) and experiment registry were saved and validated before closing the live broker; retained graph handles are now historical. Full RDF source payloads were not durably exported. There were no further public reads after the bounded four-resource acquisition and no raw Fetch, private data, configuration or active skill changes.

Reproduce offline eligibility and tests:

```sh
node scripts/wiki-learning/validate-corpus.mjs
node --test test/wiki-learning/*.test.mjs
npm run evaluation:results:validate
```

The new six-test suite includes relabeling exclusion, actual success/failure evidence, mixed scope, source-dependent finding metadata, missing evidence, fabricated anchors and rehashed-output mismatch. The 22 combined focused tests pass. Delivery starts at `1297af5159e02399baef56f4f2c57877f6d41889` on `codex/scientific-corpus-boundary`, in `/Users/cvardema/dev/git/LA3D/agents/linked-science-cloud`. The unrelated `.codex/config.toml` edit remains excluded; no push is authorized.

Scientific-corpus verification: all 22 focused wiki tests plus 3 registry tests pass. Final full `npm test`: 241/242 pass; only the pre-existing old-checkout-path assertion at `test/cleanroom-linked-science-bootstrap.test.mjs:69` remains. Smoke, corpus filtering, experiment-result registry validation, 17 receipt-file hash checks, original planning-episode hash preservation, changed-document relative links and whitespace checks pass. No active skill files changed.


## 15. Phase-2 scientific wiki maintenance implementation

The [repository wiki](../../wiki/README.md), [versioned pattern contract](../../schemas/wiki-learning/pattern.schema.json), [proposal contract](../../schemas/wiki-learning/wiki-proposal.schema.json) and [coordinator](../../lib/wiki-learning/wiki-revisions.mjs) are implemented. The [maintainer skill](../../.agents/skills/scientific-wiki-maintainer/SKILL.md) follows normal implicit discovery conventions; actual discovery in a fresh host catalog remains unobserved. The existing Linked Data REPL skill is unchanged.

Bounded proposals select explicit eligible scientific corpus units, preserve event/evidence/operation references, and declare observed success/failure strata and omissions. The coordinator reuses corpus and episode validation, checks the base revision, records review authorization when required, and keeps contradictions and supersessions inactive in the same revision. Immutable revision directories plus atomic `wiki/HEAD` replace the initially sketched mutable flat pages so interruption cannot publish mismatched pages/index/history. Missing or corrupt evidence blocks validation. Review records represent caller-declared authority, not identity authentication.

The [saved seed proposal](../../artifacts/wiki-learning/proposals/ontology-membership-seed.json) was applied once. Its two patterns remain **proposed**, with `candidate-not-generalized`: retained-query recovery after a display-limit failure, and the separate dated GO-membership finding. No independent counterexample or transfer evaluation exists. Thus the infrastructure and initial extraction are complete, but the earlier acceptance target of a reviewed hypothesis with independent counterexample evidence is not met. No reviewed scientific guidance is claimed.

The [seven regression tests](../../test/wiki-learning/wiki-revisions.test.mjs) cover stale proposals, review records, coordinated supersession/contradiction, evidence eligibility/integrity, engineering exclusion, interrupted writes, concurrent writers, canonical-file tampering and unsafe paths. See the [delivery handoff](scientific-wiki-maintainer-handoff.md) for verification and remaining boundaries. These are software/provenance records, not a new scientific experiment or registry entry. No new acquisition, Obsidian write, proposer, runtime memory injection, active-skill update, hook or release controller was added.
...
## 19. Agent-initiated wiki learning — 2026-09-23

The user authorized bounded project-local learning during normal scientific work. The Linked Data REPL skill now recognizes corrected assumptions, useful recoveries and reusable methods and routes available episode evidence to the existing maintainer. The maintainer can inspect and apply in-scope proposals without per-lesson confirmation. Prefer strengthening, qualifying, correcting or contradicting existing patterns; add a concise new pattern only for a distinct decision. Episodic evidence remains optional depth behind the canonical lesson. Provisional knowledge may be saved and consulted without a performance/activation gate; active-skill evolution and promotion remain separate.

The revision reference documents the actual capture/import/corpus path and its limitation: the current scientific adapter requires direct project-stdio witnesses and verified joins. Selected-history metadata alone cannot qualify. Unsupported formats or missing witnesses produce a bounded handoff gap, not fabricated evidence or extra acquisition. Sensitive/authenticated access, external writes, bulk acquisition/export and evaluations retain separate authority. No pattern, corpus record, runtime code, global configuration or frozen baseline was changed for this implementation. No scientific acquisition or learning trial was performed; natural activation remains to be observed during real use.

Verification: both skill-creator syntax checks, changed-document relative paths, stale-routing review, complete diff review, `git diff --check`, current wiki citation validation (14 patterns), fixed seed corpus validation, 20 corpus/revision/retrieval regression tests and six selected-history import tests pass. The retrieval baseline test now checks historical snapshot integrity while allowing the active skill to evolve, instead of requiring equality with the 2026-09-19 snapshot. `npm test` and `npm run linked-science:verify` stop at the existing checkout-path boundary mismatch; `npm run smoke` cannot resolve the absent `@comunica/query-sparql` dependency. The broader wiki test glob also cannot load phase0 without `n3`. No dependencies were installed and no configuration was changed to mask those limits.

Delivery: managed checkout `/Users/cvardema/.codex/worktrees/1b51/codex-repl`, branch `codex/agent-initiated-wiki-learning`, starting commit `7a393320fcc00dd84eaa290a536a102c7ee4a177`. Implementation commit `702770ba92ac8ddd76b2a65a7749944ef576696e` was safely fast-forwarded into Studio's authoritative saved main after the user additionally authorized version alignment. The saved checkout had been at `bd4c5bbade77fe73b0835013a6b1f00abef9af8b`, three commits behind the shared starting base and cached origin/main. Those three commits add documentation, wiki and evidence records, with no runtime or skill differences. The laptop task independently reported the same starting base, so implementation did not begin from divergent source versions.

Final verification in the configured authoritative checkout: **288/288 `npm test` tests pass**, with no skips, after allowing local Unix sockets denied by the sandbox on the first attempt. `npm run smoke` and `npm run linked-science:verify` also pass. This resolves the managed-worktree environment blockers above without installing dependencies or editing configuration. Both task commits are to be retained on local main; no push is authorized. Studio's unrelated `.codex/config.toml`, structure-viewer artifacts and consultation receipt remain untouched. The laptop alignment task is authorized to fetch the final commits directly from Studio and fast-forward after checking its own unrelated changes. Cross-host message delivery from Studio returned “no rollout found”; the final response supplies the exact handoff revision for the originating task to read. Natural learning activation remains to be observed during a future real scientific task; no staged learning trial is required.
