...
- **CodeAct runtime:** persistent model-written JavaScript uses the stable `linkedScience` facade for broker-mediated general resources, Response-like JSON/text/XML/CSV/binary handling, RDF/JS datasets, local and remote Communica queries, generic derivation, bounded views, PEEK orientation, and explicit stale-handle recovery.
