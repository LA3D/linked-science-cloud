...
- **CodeAct runtime:** persistent model-written JavaScript uses the stable `linkedScience` facade for local graph objects, schema search, Communica queries, generic derivation, bounded views, PEEK orientation, and explicit stale-handle recovery. The separately saved MCP has now been observed after restart to inject the optional named-profile broker, deny raw child DNS/network/filesystem access, and retain a bounded approved UniProt `ASK` preflight as a native boolean handle. That transport preflight is not a competency result.
...
- **Externally authored competency evaluation:** the official UniProt SPARQL example catalog is the held-out question corpus for staged clean-room evaluation. A dated catalog snapshot and three evaluator-private references now back the public non-dispatchable worker draft; official queries remain outside the worker checkout. See the [methodology](docs/experiments/uniprot-competency-question-evaluation.md). No competency case has been run, and the draft remains blocked on reviewed orientation profiles.
