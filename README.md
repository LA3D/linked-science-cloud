...
- **Schema affordances:** a resource-neutral planning surface loads versioned documentation packs. The initial UniProt/Rhea/Wikidata pack is an exemplar: a worker selects known prefixes and official query motifs, then validates a bounded non-executing plan before it constructs a query. Add another Linked Data source through its own reviewed pack and explicit endpoint profile—not by changing the generic planner. See the [affordance experiment](docs/experiments/affordance-catalog.md).
