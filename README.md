...
- **CodeAct runtime:** persistent model-written JavaScript uses the stable `linkedScience` facade for local graph objects, schema search, Communica queries, generic derivation, bounded views, PEEK orientation, and explicit stale-handle recovery. An optional named-profile broker contract now retains injected offline acquisition/query results as native handles; the separately saved MCP has not yet been shown to inject or enforce that live capability.
...
- **Externally authored competency evaluation:** the official UniProt SPARQL example catalog is the proposed held-out question corpus for staged clean-room evaluation. Workers receive natural questions while official queries remain evaluator-only references. See the [methodology](docs/experiments/uniprot-competency-question-evaluation.md); no manifest or clean-room live run exists yet.
