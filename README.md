...
- **Open-world mediated traversal:** local Communica may dereference dynamically discovered public HTTPS RDF resources and federate across SPARQL services. Every request crosses the parent mediator, which enforces DNS/SSRF, identity stripping, mutation denial, redirects, fan-out, concurrency, time, byte, and item bounds and records per-hop lineage.
...
Live use remains explicitly approved and traversal-bound. The approval fixes behavior and resource budgets, not a predeclared destination list. The isolated child has no raw networking; local Communica can reach dynamically discovered credential-free public HTTPS RDF/SPARQL targets only through the parent mediator. Mutations, unsafe DNS/address ranges, ambient identity, opaque compression, unbounded redirects/federation, and unbounded result handling are denied. Evaluation secrecy remains a separate worker-context and filesystem boundary.
