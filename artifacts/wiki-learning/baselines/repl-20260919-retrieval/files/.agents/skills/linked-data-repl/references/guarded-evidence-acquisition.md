...
Use `workspace.resources.get(url)` for bounded resource retrieval and `workspace.traversal.query` for SPARQL/Communica reads. A resource response retains a private broker receipt, media type, bytes, and a stable handle; use `resource.rdf({ name })` to parse a remote RDF representation directly, or `resources.inspect(resource, { as: 'json'|'text'|'xml'|'csv'|'binary' })` for a prompt-bounded view. Prefer a direct remote subgraph query when one small slice is sufficient; acquire and parse once when multiple local queries or RDF/JS transformations will reuse the graph. Do not reject an RDF source merely because its quad count would be large in prompt form. `HEAD` or `Content-Length` may be used as advisory evidence, but actual mediated response accounting enforces bytes and time. Each invocation creates one bounded private mediator session, returns or throws only after that session is finished or aborted, and appends a compact receipt to `workspace.traversal.history()`. Repeated calls are ordinary explicit agent attempts; the runtime does not enroll plans, impose evaluation single-shot policy, or keep a reasoning-phase timer alive between calls.

Local consumer-owned Communica receives agent-selected HTTP/HTTPS RDF sources and may execute `SERVICE` federation, while every request crosses a module-private parent mediator. The agent receives neither Fetch nor the bridge. The mediator strips ambient identity, accepts only read operations, and enforces request, fan-out, concurrency, duration, byte, and binding-item ceilings; complete large graph results use the private broker storage-byte plane when it is available. These are operational controls: the runtime never imposes a SPARQL `LIMIT`, and it retains a result handle only after the native stream or spool commit completes. Exceeding a ceiling is a failed attempt with no partial handle. Transport libraries make no hidden retry.
