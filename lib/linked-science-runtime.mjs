...
function createWorkspace(runtime, context) {
  ...
  async function updateOrientationForHandle(handle, record, role) {
    // Inventory belongs to the registry. Cache only reusable source context,
    // replacing a source's earlier description instead of adding every result.
    ...
    const version = resource ? record.value.sha256 : record.fingerprints.at(-1) ?? compactHash(record.value);
    const schema = [ 'ontology', 'schema', 'shacl' ].includes(record.kind);
    const section = schema ? 'parsing-schema' : 'context-roadmap';
    const reference = {
      kind: 'linked-science-context-reference', source, version,
    ...
    if (runtime.peek) {
      await runtime.peek.edit(context.orientation.key, [ {
        action: 'ADD', entry: {
          id: `ls-context:${compactHash(`${record.kind}\0${source}`).slice(0, 24)}`,
          section, text: JSON.stringify(reference), score: schema ? 0.8 : 0.7,
        },
      } ]);
    } else {
      context.orientation.map ??= createOrientationMap({ contextId: context.orientation.key });
      context.orientation.map = recordOrientation(context.orientation.map, {
        section, key: `${record.kind}:${compactHash(source)}`, kind: schema ? 'parsing-rule' : 'source',
        value: { source, version, role: role ?? handle.label, type: record.kind,
          handle: handle.id, 'handle-epoch': epoch, 'context-key': context.key },
        evidenceHandles: [ handle.id ], priority: schema ? 80 : 70,
      });
    }
    resolve(handle); // a reset/release during map maintenance cannot publish a live handle
  }
  ...
  async function orientationBootstrap({ checkpoint, maxItems = 30 } = {}) {
    ...
    if (runtime.peek) {
      ...
      const map = await runtime.peek.begin(context.orientation.key, { tokenBudget: Math.max(64, Math.min(32_000, maxItems * 64)) });
      return orientationStatusFromMap(map);
    }
    if (checkpoint !== undefined) context.orientation.map = recoverOrientationMap(checkpoint);
    else if (!context.orientation.map) context.orientation.map = createOrientationMap({ contextId: context.orientation.key, maxItems });
  ...
  async function orientationStatus() {
    ...
    const entries = Object.values(map.sections).flatMap(entries => entries.map(entry => ({
      id: 'ls-context:' + entry.id, text: JSON.stringify({ kind: 'linked-science-context-reference',
