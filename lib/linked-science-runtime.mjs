...
const QUERY_PLAN_BRAND = Symbol('linked-science-query-plan');
...
const EVIDENCE_KINDS = new Set([ 'grounding-evidence' ]);
const QUERY_TYPES = new Set([ 'SELECT', 'ASK', 'CONSTRUCT', 'DESCRIBE' ]);
const LOCAL_VALIDATION_FAILURE_LIMIT = 2;
const DEFAULT_SCIENTIFIC_QUERY_LIMIT = 4;
const HARD_SCIENTIFIC_QUERY_LIMIT = 20;
...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    summary: 'Execute an enrolled immutable scientific query plan through consumer-owned Communica while every explicit attempt shares one cumulative parent-mediator budget.',
    signature: 'await workspace.traversal.query(plan) -> retained native RDF/JS result handle',
    constraints: [ 'plan was created by workspace.grounding.plan after attestation', 'plan was enrolled before execution', 'each explicit attempt is receipted and bounded by iterationPolicy.maxScientificQueries', 'all attempts share cumulative request, fan-out, duration, byte, and item budgets', 'read-only SELECT/ASK/CONSTRUCT/DESCRIBE', 'credential-free HTTP/HTTPS source IRIs', 'bounded identity-free content negotiation', 'no raw Fetch/Response authority', 'no hidden transport retries' ],
  },
  grounding: {
    summary: 'Executable resource-neutral gate for discovery, evidence attestation, and scientific query planning before scored traversal.',
    methods: [ 'await workspace.grounding.begin({ target, budgets? })', 'await workspace.grounding.load({ name, document })', 'await workspace.grounding.load({ name, document, source: { kind, id } })', 'workspace.grounding.use(handle)', 'await workspace.grounding.discover({ sparql, sources, negotiation?, role? })', 'await workspace.grounding.finish()', 'workspace.grounding.attest({ evidence, sourceChoices, graphChoices, predicateChoices })', 'workspace.grounding.plan({ sparql, sources, negotiation?, role? })', 'workspace.grounding.status()' ],
    defaults: { embeddedEvidenceSource: "Omit source to use { kind: 'declarative-resource-manifest', id: name }" },
    constraints: [ 'local validation failures expose bounded structured repair metadata without consuming traversal requests', 'schema, vocabulary, and dataset evidence are required', 'all evidence references must be resident typed handles', 'source, graph, and predicate choices cite evidence handles', 'discovery uses a distinct cumulative mediator session', 'grounded evidence is registered as a bounded RLM context', 'an initial immutable plan exists before traversal.begin starts the scored session', 'later immutable plan revisions require traversal.enroll and stay in that same scored session', 'grounded outputs may seed later resource grounding through the same typed contract', 'resource-specific facts remain declarative data' ],
  },
  'traversal.exploration': {
    summary: 'Attach one cumulative anonymous-read authority and policy scope to explicit scientific iterations across several agent decisions.',
    methods: [ 'await workspace.traversal.begin({ plans, budgets?, iterationPolicy? })', 'workspace.traversal.enroll(plan)', 'await workspace.traversal.query(plan)', 'await workspace.traversal.status()', 'await workspace.traversal.finish()', 'await workspace.traversal.abort(reason?)' ],
    constraints: [ 'one active scored exploration per goal workspace', 'iterationPolicy.maxScientificQueries defaults to a bounded multi-attempt allowance and may be set to 1 by an evaluation', 'new immutable plan revisions are explicitly enrolled and receipted', 'all attempts share cumulative request, source, byte, duration, and result budgets', 'transport libraries perform no hidden retries', 'raw or unplanned query calls are rejected' ],
  },
  'results.derive': {
  ...
  recovery: {
    summary: 'Errors expose code, stage, receipt, recoveryDocument, retryable, and structured repair fields. Repair scoped local-validation failures in place; reset only for actual runtime invalidation.',
    ...
    localValidation: [ 'inspect error.repair.field and error.repair.expected', 'correct the call while repair.remaining > 0', 'local repairs consume no discovery or scored request budget' ],
  },
  security: {
    ...
    brokerRule: 'The child has no raw transport. A private Fetch closure is injected only into consumer-owned Communica calls; dynamic anonymous HTTP/HTTPS reads cross token-bound mediator sessions with behavior and aggregate resource controls.',
  },
  compatibility: {
...
export const LINKED_SCIENCE_API_SCHEMA = Object.freeze({
  ...
  workspace: {
    ...
    schema: [ 'search' ],
    query: [ 'run', 'select' ],
    grounding: [ 'begin', 'load', 'use', 'discover', 'finish', 'attest', 'plan', 'status' ],
    traversal: [ 'begin', 'enroll', 'query', 'status', 'finish', 'abort' ],
...
function createWorkspace(runtime, context) {
  ...
  const localValidationFailures = new Map();

  const nextOperation = () => `op-${String(++operationSequence).padStart(6, '0')}`;

  function assertLocalRepairAvailable(phase) {
    const failures = localValidationFailures.get(phase) ?? 0;
    if (failures >= LOCAL_VALIDATION_FAILURE_LIMIT) {
      throw runtimeError('LS_LOCAL_REPAIR_LIMIT', phase, `Local validation repair allowance is exhausted for ${phase}`, {
        recoveryDocument: 'recovery',
        retryable: false,
        repair: {
          kind: 'linked-science-local-validation-repair', scope: 'local-validation', phase,
          allowed: false, failures, limit: LOCAL_VALIDATION_FAILURE_LIMIT, remaining: 0,
          budgetImpact: { discoveryRequests: 0, scoredRequests: 0 },
        },
      });
    }
  }
  ...
  function localValidationError(code, phase, message, { field, expected, action } = {}) {
    const failures = (localValidationFailures.get(phase) ?? 0) + 1;
    localValidationFailures.set(phase, failures);
    const remaining = Math.max(0, LOCAL_VALIDATION_FAILURE_LIMIT - failures);
    const repair = {
      kind: 'linked-science-local-validation-repair', scope: 'local-validation', phase,
      allowed: remaining > 0, failures, limit: LOCAL_VALIDATION_FAILURE_LIMIT, remaining,
    ...
    return runtimeError(code, phase, message, {
      ...
      recoveryDocument: 'recovery', retryable: remaining > 0, repair,
    });
  }
  ...
  function validateGroundingChoice(choice, name, evidenceIds) {
    if (!choice || typeof choice !== 'object' || typeof choice.term !== 'string' || choice.term.length < 1 || choice.term.length > 2_048) {
      throw localValidationError('LS_GROUNDING_CHOICE', 'grounding-attestation', `${name} choices require a bounded term`, { field: `${name.toLowerCase()}Choices[].term`, expected: { type: 'string', minLength: 1, maxLength: 2048 } });
    }
    if (!Array.isArray(choice.evidenceHandles) || choice.evidenceHandles.length < 1 || choice.evidenceHandles.some(handle => !handle || !evidenceIds.has(handle.id))) {
      throw localValidationError('LS_GROUNDING_CHOICE', 'grounding-attestation', `${name} choices must cite resident attested evidence handles`, { field: `${name.toLowerCase()}Choices[].evidenceHandles`, expected: { residentAttestedHandles: true, minItems: 1 } });
    }
    return { term: choice.term, evidenceHandles: [ ...new Set(choice.evidenceHandles.map(handle => handle.id)) ] };
  }

  async function beginGrounding({ target, budgets = {} } = {}) {
    ensureActive();
    if (!runtime.traversal || !runtime.traversalCapabilities) throw runtimeError('LS_TRAVERSAL_UNAVAILABLE', 'grounding-discovery', 'No clean-room traversal mediator is available', { retryable: true, recoveryDocument: 'security' });
    if (grounding?.phase === 'discovering') throw runtimeError('LS_GROUNDING_ACTIVE', 'grounding-discovery', 'This workspace already has active grounding discovery');
    if (exploration) throw runtimeError('LS_EXPLORATION_ACTIVE', 'grounding-discovery', 'A scored exploration is already active');
    assertLocalRepairAvailable('grounding-preflight');
    if (typeof target !== 'string' || target.length < 1 || target.length > 512) {
      throw localValidationError('LS_GROUNDING_TARGET', 'grounding-preflight', 'Grounding requires a bounded resource-neutral target description', {
        field: 'target', expected: { type: 'string', minLength: 1, maxLength: 512 }, action: 'Correct target and call grounding.begin again.',
      });
    }
    let begun;
    try {
      ...
      throw localValidationError('LS_GROUNDING_BUDGET', 'grounding-preflight', error.message, { field: 'budgets', expected: { boundedBy: runtime.traversalCapabilities.hardBudgets }, action: 'Correct the local discovery budgets and call grounding.begin again.' });
    }
    grounding = {
      phase: 'discovering', target, traversalId: begun.traversalId, startedAt: begun.startedAt,
      effectiveBudgets: begun.effectiveBudgets, resultItems: 0, operations: 0, evidenceHandles: new Set(),
    };
    return freezeJson({ kind: 'linked-science-grounding', status: 'discovering', contextKey: context.key, target, traversalId: begun.traversalId, startedAt: begun.startedAt, effectiveBudgets: begun.effectiveBudgets });
  }

  async function loadGroundingEvidence({ name, document, source = { kind: 'declarative-resource-manifest', id: name } } = {}) {
    ensureActive();
    if (!grounding || grounding.phase !== 'discovering') throw runtimeError('LS_GROUNDING_INACTIVE', 'grounding-evidence', 'Call grounding.begin before loading evidence');
    assertLocalRepairAvailable('grounding-evidence');
    if (typeof name !== 'string' || !/^[a-z][a-z0-9-]{1,63}$/u.test(name)) {
      throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Evidence name must be a lowercase symbolic identifier', {
        field: 'name', expected: { type: 'string', pattern: '^[a-z][a-z0-9-]{1,63}$' }, action: 'Correct name and call grounding.load again.',
      });
    }
    if (!document || typeof document !== 'object' || Array.isArray(document)) {
      throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Evidence document must be a declarative object', {
        field: 'document', expected: { type: 'object', array: false }, action: 'Supply the embedded declarative evidence document and call grounding.load again.',
      });
    }
    if (!source || ![ 'declarative-resource-manifest', 'local-documentation' ].includes(source.kind) || typeof source.id !== 'string' || source.id.length < 1) {
      throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'The optional source field must use typed local provenance; omit it for the generic declarative-manifest default', {
        field: 'source',
        expected: {
          optional: true,
          oneOf: [
        ...
        action: 'Normally omit source and call grounding.load({ name, document }); otherwise supply one accepted { kind, id } object.',
      });
    }
    if (byteLength(document) > runtime.budgets.maxBytes) {
      throw localValidationError('LS_GROUNDING_EVIDENCE_BOUND', 'grounding-evidence', 'Evidence document exceeds the runtime byte bound', {
    ...
    const handle = retain('grounding-evidence', name, freezeJson(jsonClone(document)), {
      provenance: { operationId, source: { ...source }, evidenceSha256, protocolPhase: 'grounding' },
      lineage: { kind: 'declarative-grounding-evidence', operationId, evidenceSha256 },
      fingerprints: [ evidenceSha256 ],
    });
    grounding.evidenceHandles.add(handle.id);
    return handle;
  }

  function useGroundingEvidence(handle) {
    ensureActive();
    if (!grounding || grounding.phase !== 'discovering') throw runtimeError('LS_GROUNDING_INACTIVE', 'grounding-evidence', 'Call grounding.begin before selecting resident evidence');
    const record = resolve(handle, new Set([ ...EVIDENCE_KINDS, ...RESULT_KINDS, ...GRAPH_KINDS ]));
    if (!record.provenance || !record.lineage) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Grounding inputs require typed provenance and lineage');
    grounding.evidenceHandles.add(handle.id);
    return Object.freeze({ kind: 'linked-science-grounding-evidence-reference', handle, type: record.kind, fingerprint: record.fingerprints[0] });
  }

  async function finishGrounding() {
    ensureActive();
    if (!grounding || grounding.phase !== 'discovering') throw runtimeError('LS_GROUNDING_INACTIVE', 'grounding-discovery', 'No grounding discovery is active');
    const receipt = await runtime.traversal.finishTraversal(grounding.traversalId);
    grounding.discoveryReceipt = receipt;
    grounding.phase = 'evidence-ready';
    return freezeJson({ ...receipt, protocolPhase: 'grounding-discovery', goalContextKey: context.key, resultItems: grounding.resultItems, operations: grounding.operations });
  }

  function attestGrounding({ evidence, sourceChoices, graphChoices, predicateChoices } = {}) {
    ensureActive();
    if (!grounding || grounding.phase !== 'evidence-ready') throw runtimeError('LS_GROUNDING_INCOMPLETE', 'grounding-attestation', 'Finish bounded grounding discovery before attestation');
    assertLocalRepairAvailable('grounding-attestation');
    if (!Array.isArray(evidence) || evidence.length < 1) throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Grounding attestation requires resident evidence handles', { field: 'evidence', expected: { type: 'array', minItems: 1 } });
    const supports = new Set();
    const evidenceIds = new Set();
    const attestedEvidence = evidence.map(item => {
      if (!item || typeof item !== 'object' || !Array.isArray(item.supports) || item.supports.length < 1 || typeof item.locator !== 'string' || item.locator.length < 1 || item.locator.length > 1_024) {
        throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Each evidence item requires a handle, supports list, and bounded locator', { field: 'evidence[]', expected: { shape: '{ handle, supports: [schema|vocabulary|dataset], locator }' } });
      }
      const record = resolve(item.handle, new Set([ ...EVIDENCE_KINDS, ...RESULT_KINDS, ...GRAPH_KINDS ]));
      if (!grounding.evidenceHandles.has(item.handle.id)) throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Evidence handle was not acquired during this grounding phase', { field: 'evidence[].handle', expected: { residentInCurrentGrounding: true } });
      const itemSupports = [ ...new Set(item.supports) ];
      if (itemSupports.some(value => ![ 'schema', 'vocabulary', 'dataset' ].includes(value))) throw localValidationError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Evidence supports must be schema, vocabulary, or dataset', { field: 'evidence[].supports', expected: { enum: [ 'schema', 'vocabulary', 'dataset' ] } });
      itemSupports.forEach(value => supports.add(value));
      evidenceIds.add(item.handle.id);
      return { handle: item.handle.id, type: record.kind, label: item.handle.label, supports: itemSupports, locator: item.locator, provenance: record.provenance };
    });
    for (const required of [ 'schema', 'vocabulary', 'dataset' ]) if (!supports.has(required)) {
      throw localValidationError('LS_GROUNDING_INCOMPLETE', 'grounding-attestation', `Grounding lacks ${required} evidence`, { field: 'evidence[].supports', expected: { requiredRoles: [ 'schema', 'vocabulary', 'dataset' ] } });
    }
    if (!Array.isArray(sourceChoices) || sourceChoices.length < 1 || !Array.isArray(graphChoices) || graphChoices.length < 1 || !Array.isArray(predicateChoices) || predicateChoices.length < 1) {
      throw localValidationError('LS_GROUNDING_CHOICE', 'grounding-attestation', 'Grounding requires evidence-backed source, graph, and predicate choices', { field: 'sourceChoices|graphChoices|predicateChoices', expected: { each: 'non-empty array of { term, evidenceHandles }' } });
    }
    const normalizedSources = sourceChoices.map(choice => validateGroundingChoice(choice, 'Source', evidenceIds));
    const normalizedGraphs = graphChoices.map(choice => validateGroundingChoice(choice, 'Graph', evidenceIds));
    const normalizedPredicates = predicateChoices.map(choice => validateGroundingChoice(choice, 'Predicate', evidenceIds));
    const contextId = `linked-science:grounding:${compactHash(context.key).slice(0, 16)}`;
    const groundingContext = freezeJson({
      kind: 'linked-science-grounding-context', version: RUNTIME_VERSION, contextKey: context.key, target: grounding.target,
      evidence: attestedEvidence, sourceChoices: normalizedSources, graphChoices: normalizedGraphs, predicateChoices: normalizedPredicates,
      discovery: { receiptSha256: compactHash(grounding.discoveryReceipt), operations: grounding.operations, resultItems: grounding.resultItems },
    });
    if (byteLength(groundingContext) > runtime.budgets.maxBytes) throw localValidationError('LS_GROUNDING_CONTEXT_BOUND', 'grounding-attestation', 'Grounding context exceeds the runtime byte bound', { field: 'attestation', expected: { maxBytes: runtime.budgets.maxBytes } });
    runtime.contextRegistry?.registerContext(contextId, groundingContext);
    grounding.phase = 'attested';
    grounding.contextId = contextId;
    grounding.context = groundingContext;
    grounding.attestationSha256 = compactHash(groundingContext);
    return freezeJson({ kind: 'linked-science-grounding-attestation', status: 'attested', contextId, sha256: grounding.attestationSha256, evidenceHandles: [ ...evidenceIds ], sourceChoices: normalizedSources.length, graphChoices: normalizedGraphs.length, predicateChoices: normalizedPredicates.length });
  }
  ...
  function planGroundedQuery(options = {}) {
    ensureActive();
    if (!grounding || grounding.phase !== 'attested') throw runtimeError('LS_GROUNDING_REQUIRED', 'query-planning', 'Scientific query planning requires an attested grounding context');
    assertLocalRepairAvailable('query-planning');
    try {
      validateMediatedQueryOptions(options);
    } catch (error) {
      throw localValidationError('LS_QUERY_PLAN', 'query-planning', error.message, {
        field: error.field ?? 'options', expected: error.expected, action: 'Correct the immutable plan input and call grounding.plan again before enrollment.',
      });
    }
    if (queryPlans.size >= HARD_SCIENTIFIC_QUERY_LIMIT) throw localValidationError('LS_QUERY_PLAN_LIMIT', 'query-planning', 'Grounded query plan bound is exhausted', { field: 'plans', expected: { maxItems: HARD_SCIENTIFIC_QUERY_LIMIT } });
    const id = `plan-${String(queryPlans.size + 1).padStart(6, '0')}`;
    const plan = { kind: 'linked-science-query-plan', id, querySha256: compactHash(options.sparql), groundingSha256: grounding.attestationSha256 };
    Object.defineProperty(plan, QUERY_PLAN_BRAND, { value: true });
    Object.freeze(plan);
    const plannedOptions = freezeJson(jsonClone(options));
    const plannedContext = freezeJson({ ...grounding.context, plans: [ ...(grounding.context.plans ?? []), { id, querySha256: plan.querySha256, groundingSha256: plan.groundingSha256 } ] });
    if (byteLength(plannedContext) > runtime.budgets.maxBytes) throw localValidationError('LS_GROUNDING_CONTEXT_BOUND', 'query-planning', 'Grounding plan metadata exceeds the runtime byte bound', { field: 'plans', expected: { contextMaxBytes: runtime.budgets.maxBytes } });
    runtime.contextRegistry?.registerContext(grounding.contextId, plannedContext);
    grounding.context = plannedContext;
    queryPlans.set(id, { plan, options: plannedOptions, groundingSha256: grounding.attestationSha256, attempts: 0 });
    return plan;
  }

  function groundingStatus() {
    ensureActive();
    if (!grounding) return Object.freeze({ kind: 'linked-science-grounding', status: 'uninitialized', contextKey: context.key });
    return freezeJson({ kind: 'linked-science-grounding', status: grounding.phase, contextKey: context.key, target: grounding.target, contextId: grounding.contextId, attestationSha256: grounding.attestationSha256, evidenceHandles: [ ...grounding.evidenceHandles ], plans: queryPlans.size });
  }

  function normalizeIterationPolicy(policy, budgets) {
    const phase = 'scored-traversal-preflight';
    assertLocalRepairAvailable(phase);
    if (policy !== undefined && (!policy || typeof policy !== 'object' || Array.isArray(policy) || Object.keys(policy).some(key => key !== 'maxScientificQueries'))) {
      throw localValidationError('LS_ITERATION_POLICY', phase, 'iterationPolicy accepts only maxScientificQueries', {
        field: 'iterationPolicy', expected: { shape: '{ maxScientificQueries?: integer }' }, action: 'Correct the local policy before starting the scored timer.',
      });
    }
    const requested = policy?.maxScientificQueries ?? DEFAULT_SCIENTIFIC_QUERY_LIMIT;
    if (!Number.isInteger(requested) || requested < 1 || requested > HARD_SCIENTIFIC_QUERY_LIMIT) {
      throw localValidationError('LS_ITERATION_POLICY', phase, 'maxScientificQueries must be a bounded positive integer', {
        field: 'iterationPolicy.maxScientificQueries', expected: { minimum: 1, maximum: HARD_SCIENTIFIC_QUERY_LIMIT }, action: 'Choose a bounded explicit-attempt allowance; evaluations may use 1 for a single-shot case.',
      });
    }
    return Object.freeze({ maxScientificQueries: requested, hiddenTransportRetries: 0, cumulativeBudgets: true });
  }

  async function beginExploration({ plans, budgets = {}, iterationPolicy: inputPolicy } = {}) {
    ensureActive();
    if (!runtime.traversal || !runtime.traversalCapabilities) throw runtimeError('LS_TRAVERSAL_UNAVAILABLE', 'traversal-preflight', 'No clean-room traversal mediator is available', { retryable: true, recoveryDocument: 'security' });
    if (exploration) throw runtimeError('LS_EXPLORATION_ACTIVE', 'traversal-exploration', 'This goal workspace already has an active exploration');
    if (!grounding || grounding.phase !== 'attested') throw runtimeError('LS_GROUNDING_REQUIRED', 'traversal-preflight', 'Scored traversal requires completed grounding and planning');
    const iterationPolicy = normalizeIterationPolicy(inputPolicy, budgets);
    if (!Array.isArray(plans) || plans.length < 1 || plans.length > HARD_SCIENTIFIC_QUERY_LIMIT) throw localValidationError('LS_QUERY_PLAN_REQUIRED', 'scored-traversal-preflight', 'Enroll one to twenty immutable grounded plans', { field: 'plans', expected: { minItems: 1, maxItems: HARD_SCIENTIFIC_QUERY_LIMIT } });
    const enrolledPlans = plans.map(plan => {
      if (!plan || plan[QUERY_PLAN_BRAND] !== true || plan.kind !== 'linked-science-query-plan') throw localValidationError('LS_QUERY_PLAN_REQUIRED', 'scored-traversal-preflight', 'An immutable query plan from this workspace is required', { field: 'plans[]', expected: { kind: 'linked-science-query-plan', currentWorkspace: true } });
      const record = queryPlans.get(plan.id);
      if (!record || record.plan !== plan || record.groundingSha256 !== grounding.attestationSha256) throw localValidationError('LS_QUERY_PLAN_STALE', 'scored-traversal-preflight', 'Query plan is unknown or stale', { field: 'plans[]', expected: { groundedByCurrentAttestation: true } });
      return plan.id;
    });
    let begun;
    try {
      ...
      if (error?.code !== 'MEDIATOR_BUDGET_INVALID') throw error;
      throw localValidationError('LS_SCORED_BUDGET', 'scored-traversal-preflight', error.message, { field: 'budgets', expected: { boundedBy: runtime.traversalCapabilities.hardBudgets }, action: 'Correct the local cumulative budgets before starting the scientific timer.' });
    }
    exploration = { traversalId: begun.traversalId, startedAt: begun.startedAt, effectiveBudgets: begun.effectiveBudgets, iterationPolicy, resultItems: 0, operations: 0, enrolledPlans: new Set(enrolledPlans), attempts: [], revisions: [] };
    return freezeJson({ kind: 'linked-science-goal-exploration', status: 'active', protocolPhase: 'scored-traversal', contextKey: context.key, traversalId: exploration.traversalId, startedAt: exploration.startedAt, effectiveBudgets: exploration.effectiveBudgets, iterationPolicy, enrolledPlans, usage: { resultItems: 0, operations: 0, scientificQueries: 0, planRevisions: 0 } });
  }

  function enrollPlan(plan) {
    ensureActive();
    if (!exploration) throw runtimeError('LS_EXPLORATION_INACTIVE', 'query-enrollment', 'Start scored traversal before enrolling a later plan revision');
    assertLocalRepairAvailable('query-enrollment');
    if (!plan || plan[QUERY_PLAN_BRAND] !== true || plan.kind !== 'linked-science-query-plan') throw localValidationError('LS_QUERY_PLAN_REQUIRED', 'query-enrollment', 'traversal.enroll accepts one immutable grounded plan', { field: 'plan', expected: { kind: 'linked-science-query-plan', currentWorkspace: true } });
    const record = queryPlans.get(plan.id);
    if (!record || record.plan !== plan || record.groundingSha256 !== grounding.attestationSha256) throw localValidationError('LS_QUERY_PLAN_STALE', 'query-enrollment', 'Query plan is unknown or stale', { field: 'plan', expected: { groundedByCurrentAttestation: true } });
    if (exploration.enrolledPlans.has(plan.id)) return freezeJson({ kind: 'linked-science-plan-enrollment', status: 'already-enrolled', planId: plan.id, querySha256: plan.querySha256, revision: exploration.revisions.length });
    if (exploration.enrolledPlans.size >= HARD_SCIENTIFIC_QUERY_LIMIT) throw localValidationError('LS_QUERY_PLAN_LIMIT', 'query-enrollment', 'Enrolled plan bound is exhausted', { field: 'plan', expected: { maxEnrolledPlans: HARD_SCIENTIFIC_QUERY_LIMIT } });
    exploration.enrolledPlans.add(plan.id);
    const receipt = freezeJson({ kind: 'linked-science-plan-enrollment', status: 'enrolled', planId: plan.id, querySha256: plan.querySha256, groundingSha256: plan.groundingSha256, revision: exploration.revisions.length + 1, traversalId: exploration.traversalId });
    exploration.revisions.push(receipt);
    return receipt;
  }

  async function explorationStatus() {
    ensureActive();
    if (!exploration) return Object.freeze({ kind: 'linked-science-goal-exploration', status: 'inactive', contextKey: context.key });
    const receipt = await runtime.traversal.snapshotTraversal(exploration.traversalId);
    return freezeJson({ kind: 'linked-science-goal-exploration', status: 'active', contextKey: context.key, traversalId: exploration.traversalId, iterationPolicy: exploration.iterationPolicy, usage: { ...receipt.usage, resultItems: exploration.resultItems, operations: exploration.operations, scientificQueries: exploration.attempts.length, planRevisions: exploration.revisions.length }, attempts: [ ...exploration.attempts ], revisions: [ ...exploration.revisions ], budgets: receipt.budgets, navigation: navigationEvidence(receipt) });
  }

  async function finishExploration() {
    ensureActive();
    if (!exploration) throw runtimeError('LS_EXPLORATION_INACTIVE', 'traversal-exploration', 'This goal workspace has no active exploration');
    const current = exploration;
    const receipt = await runtime.traversal.finishTraversal(current.traversalId);
    exploration = undefined;
    return freezeJson({ ...receipt, goalContextKey: context.key, iterationPolicy: current.iterationPolicy, resultItems: current.resultItems, operations: current.operations, scientificQueries: current.attempts.length, planRevisions: current.revisions.length, attempts: [ ...current.attempts ], revisions: [ ...current.revisions ], navigation: navigationEvidence(receipt) });
  }

  async function abortExploration(reason = 'agent-stopped') {
    ensureActive();
    if (!exploration) return Object.freeze({ kind: 'linked-science-goal-exploration', status: 'inactive', contextKey: context.key });
    const current = exploration;
    exploration = undefined;
    const receipt = await runtime.traversal.abortTraversal(current.traversalId, String(reason).slice(0, 96));
    return freezeJson({ ...receipt, goalContextKey: context.key, iterationPolicy: current.iterationPolicy, resultItems: current.resultItems, operations: current.operations, scientificQueries: current.attempts.length, planRevisions: current.revisions.length, attempts: [ ...current.attempts ], revisions: [ ...current.revisions ], navigation: navigationEvidence(receipt) });
  }

  async function executeMediatedQuery({ sparql, sources, negotiation = {}, role } = {}, scope, protocolPhase, attempt) {
    ensureActive();
    ...
    try {
      ...
      if (protocolPhase === 'grounding-discovery') {
        assertLocalRepairAvailable('grounding-discovery-preflight');
        throw localValidationError('LS_GROUNDING_DISCOVERY_PREFLIGHT', 'grounding-discovery-preflight', error.message, {
          field: error.field ?? 'options', expected: error.expected, action: 'Correct the local discovery call; no request was made and the current discovery budget remains active.',
        });
      }
      throw wrapError(error, 'LS_TRAVERSAL_PREFLIGHT', 'traversal-preflight', { operationId, ...(attempt ? { attempt: { ...attempt, status: 'failed', failureCode: 'LS_TRAVERSAL_PREFLIGHT' } } : {}) });
    }
    let traversalId;
    try {
      traversalId = scope.traversalId;
      const effectiveBudgets = scope.effectiveBudgets;
      ...
      const remainingResultItems = effectiveBudgets.maxResultItems - scope.resultItems;
      if (remainingResultItems < 1) throw runtimeError('LS_EXPLORATION_RESULT_BOUND', 'traversal-result', 'Goal exploration result-item budget is exhausted');
      const queryContext = {
      ...
      if (parsed.queryType === 'SELECT') {
        kind = 'bindings';
        value = normalizeBindings(await collectBounded(await engine.queryBindings(sparql, queryContext), remainingResultItems, 'LS_TRAVERSAL_RESULT_BOUND', 'traversal-result'));
      } else if (parsed.queryType === 'ASK') {
        kind = 'boolean';
        value = await engine.queryBoolean(sparql, queryContext);
      } else {
        kind = 'quads';
        value = new Store((await collectBounded(await engine.queryQuads(sparql, queryContext), remainingResultItems, 'LS_TRAVERSAL_RESULT_BOUND', 'traversal-result')).map(copyQuad));
      }
      const resultItems = kind === 'quads' ? value.size : kind === 'boolean' ? 1 : value.length;
      scope.resultItems += resultItems;
      scope.operations += 1;
      const receipt = await runtime.traversal.snapshotTraversal(traversalId);
      if (receipt?.kind !== 'linked-science-traversal-receipt' || receipt.status !== 'active' || receipt.traversalId !== traversalId) {
      ...
      const provenance = {
        operationId, localOnly: false, mediatedTraversal: true, protocolPhase, queryType: parsed.queryType, querySha256,
        ...(attempt ? { attempt: { ...attempt, status: 'success', operationId } } : {}),
        sources: mediatedSources,
        negotiation: Object.fromEntries([ ...sourceNegotiations ].map(([ source, headers ]) => [ source, Object.fromEntries(headers) ])),
        traversalReceipt: receipt, navigation, payloadSha256,
      };
      const handle = retain(kind, role ?? `${parsed.queryType.toLowerCase()}-traversal-result`, value, {
        provenance,
        lineage: { kind: 'communica-mediated-traversal', operationId, traversalId, querySha256, payloadSha256 },
        fingerprints: [ payloadSha256 ],
      });
      if (protocolPhase === 'grounding-discovery') grounding.evidenceHandles.add(handle.id);
      return handle;
    } catch (error) {
      ...
      throw wrapError(error, 'LS_TRAVERSAL_QUERY', 'traversal-query', {
        operationId, traversalId, traversalReceipt, navigation,
        ...(attempt ? { attempt: { ...attempt, status: 'failed', operationId, failureCode: error.code ?? 'LS_TRAVERSAL_QUERY' } } : {}),
      });
    }
  }

  async function discoverGrounding(options = {}) {
    ensureActive();
    if (!grounding || grounding.phase !== 'discovering') throw runtimeError('LS_GROUNDING_INACTIVE', 'grounding-discovery', 'Call grounding.begin before bounded discovery');
    return executeMediatedQuery(options, grounding, 'grounding-discovery');
  }

  async function traverseQuery(plan) {
    ensureActive();
    if (!runtime.traversal || !runtime.traversalCapabilities) throw runtimeError('LS_TRAVERSAL_UNAVAILABLE', 'traversal-preflight', 'No clean-room traversal mediator is available', { retryable: true, recoveryDocument: 'security' });
    if (!exploration) throw runtimeError('LS_EXPLORATION_INACTIVE', 'traversal-preflight', 'Start scored traversal with pre-grounded plans before executing a scientific query');
    assertLocalRepairAvailable('scored-query-preflight');
    if (!plan || plan[QUERY_PLAN_BRAND] !== true || plan.kind !== 'linked-science-query-plan') throw localValidationError('LS_QUERY_PLAN_REQUIRED', 'scored-query-preflight', 'traversal.query accepts only an immutable grounded plan', { field: 'plan', expected: { kind: 'linked-science-query-plan', enrolled: true } });
    const record = queryPlans.get(plan.id);
    if (!record || record.plan !== plan || !exploration.enrolledPlans.has(plan.id)) throw localValidationError('LS_QUERY_PLAN_STALE', 'scored-query-preflight', 'Query plan is unknown, stale, or not enrolled', { field: 'plan', expected: { currentWorkspace: true, enrolled: true } });
    if (exploration.attempts.length >= exploration.iterationPolicy.maxScientificQueries) {
      throw runtimeError('LS_SCIENTIFIC_ITERATION_LIMIT', 'scored-scientific-query', 'Scientific query iteration allowance is exhausted', {
        receipt: { status: 'failed', stage: 'scored-scientific-query', code: 'LS_SCIENTIFIC_ITERATION_LIMIT', iterationPolicy: exploration.iterationPolicy, attempts: [ ...exploration.attempts ] },
        retryable: false,
      });
    }
    record.attempts += 1;
    const attempt = freezeJson({
      kind: 'linked-science-scientific-query-attempt', index: exploration.attempts.length + 1,
      planId: plan.id, planAttempt: record.attempts, querySha256: plan.querySha256,
      groundingSha256: plan.groundingSha256, traversalId: exploration.traversalId,
    });
    try {
      const handle = await executeMediatedQuery(record.options, exploration, 'scored-scientific-query', attempt);
      const completed = freezeJson({ ...attempt, status: 'success', operationId: resolve(handle).provenance.operationId });
      exploration.attempts.push(completed);
      return handle;
    } catch (error) {
      const failed = freezeJson(error.receipt?.attempt ?? { ...attempt, status: 'failed', failureCode: error.code ?? 'LS_TRAVERSAL_QUERY' });
      exploration.attempts.push(failed);
      throw error;
    }
  }
  ...
  const workspace = {
    ...
    grounding: Object.freeze({ begin: beginGrounding, load: loadGroundingEvidence, use: useGroundingEvidence, discover: discoverGrounding, finish: finishGrounding, attest: attestGrounding, plan: planGroundedQuery, status: groundingStatus }),
    traversal: Object.freeze({ begin: beginExploration, enroll: enrollPlan, query: traverseQuery, status: explorationStatus, finish: finishExploration, abort: abortExploration }),
  ...
  Object.defineProperty(workspace, '_invalidate', { value() {
    if (exploration) Promise.resolve(runtime.traversal.abortTraversal(exploration.traversalId, 'workspace-invalidated')).catch(() => {});
    if (grounding?.phase === 'discovering') Promise.resolve(runtime.traversal.abortTraversal(grounding.traversalId, 'workspace-invalidated')).catch(() => {});
    exploration = undefined;
    grounding = undefined;
    queryPlans.clear();
    active = false;
...
function createFacade({ nodeRepl, budgets, orientationCheckpoints = {}, peek, environment, traversal, traversalCapabilityReceipt, contextRegistry }) {
  const runtimeId = randomUUID();
  const contexts = new Map();
  const documentation = docsFunction();
  const runtime = { runtimeId, budgets: validateBudgets(budgets), peek, traversal, traversalCapabilities: traversalCapabilityReceipt, contextRegistry };
    ...
    capabilities() {
      return Object.freeze({
        ...
        iteration: Object.freeze({
          localValidationFailureLimit: LOCAL_VALIDATION_FAILURE_LIMIT,
          defaultMaxScientificQueries: DEFAULT_SCIENTIFIC_QUERY_LIMIT,
          hardMaxScientificQueries: HARD_SCIENTIFIC_QUERY_LIMIT,
          explicitAgentAttempts: 'policy-driven-cumulative-budget',
...
export async function setupLinkedScience({ nodeRepl, budgets, orientationCheckpoints, peek, environment, traversal, contextRegistry } = {}) {
  ...
  if (contextRegistry !== undefined && typeof contextRegistry?.registerContext !== 'function') throw runtimeError('LS_CONTEXT_REGISTRY_REQUIRED', 'bootstrap', 'The clean-room context registry must expose registerContext');
  const traversalCapabilityReceipt = await traversalCapabilities(traversal);
  const facade = createFacade({ nodeRepl, budgets, orientationCheckpoints, peek, environment, traversal, traversalCapabilityReceipt, contextRegistry });
