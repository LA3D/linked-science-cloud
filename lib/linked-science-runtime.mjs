...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    summary: 'Run local Communica over dynamically selected public HTTPS RDF/SPARQL sources while every request crosses the parent mediator.',
    signature: "await workspace.traversal.query({ sparql, sources: [httpsIri | { type: 'sparql', value: httpsIri }], budgets?, role? }) -> result handle",
    constraints: [ 'read-only SELECT/ASK/CONSTRUCT/DESCRIBE', 'credential-free HTTPS source IRIs', 'aggregate and per-hop receipts', 'bounded duration, fan-out, concurrency, bytes, and items' ],
  },
  'results.derive': {
  ...
  security: {
    ...
    brokerRule: 'The child has no raw transport. Communica sends each public HTTPS RDF/SPARQL request through token-bound mediator sessions with behavior and aggregate resource controls.',
  },
  compatibility: {
...
async function traversalCapabilities(traversal) {
  ...
  if (!capabilities || capabilities.kind !== 'linked-science-traversal-mediator' || typeof capabilities.version !== 'string' ||
    !Array.isArray(capabilities.methods) || !Array.isArray(capabilities.queryTypes) || !Array.isArray(capabilities.mediaTypes) ||
    !capabilities.hardBudgets || capabilities.networkPolicy !== 'public-https-dns-pinned' || capabilities.retries !== 0) {
    throw runtimeError('LS_TRAVERSAL_CAPABILITIES', 'bootstrap', 'Traversal mediator returned an invalid capability receipt');
  }
  if (capabilities.methods.some(method => ![ 'GET', 'HEAD', 'POST' ].includes(method)) ||
    capabilities.queryTypes.some(type => !QUERY_TYPES.has(type))) {
...
function createWorkspace(runtime, context) {
  ...
  async function traverseQuery({ sparql, sources, role, budgets = {} } = {}) {
    ensureActive();
    ...
    try {
      ...
      if (!Array.isArray(sources) || sources.length < 1 || sources.length > 20) throw new Error('One to twenty public HTTPS source IRIs are required');
      mediatedSources = sources.map(source => {
        ...
        if (url.protocol !== 'https:' || url.username || url.password || url.hash) throw new Error('Traversal sources must be credential-free HTTPS IRIs without fragments');
    ...
    try {
      ...
      const fetch = runtime.traversal.createFetch(traversalId);
      let kind;
      let value;
      if (parsed.queryType === 'SELECT') {
        kind = 'bindings';
        value = normalizeBindings(await (await engine.queryBindings(sparql, { sources: mediatedSources, fetch })).toArray());
      } else if (parsed.queryType === 'ASK') {
        kind = 'boolean';
        value = await engine.queryBoolean(sparql, { sources: mediatedSources, fetch });
      } else {
        kind = 'quads';
        value = (await (await engine.queryQuads(sparql, { sources: mediatedSources, fetch })).toArray()).map(copyQuad);
      }
      if ((Array.isArray(value) ? value.length : 1) > begun.budgets.maxResultItems) throw runtimeError('LS_TRAVERSAL_RESULT_BOUND', 'traversal-result', 'Communica result exceeds the traversal item bound');
      ...
      const querySha256 = compactHash(sparql);
      const payloadSha256 = compactHash(value);
      const provenance = {
        operationId, localOnly: false, mediatedTraversal: true, queryType: parsed.queryType, querySha256,
        sources: mediatedSources, traversalReceipt: receipt, payloadSha256,
      };
