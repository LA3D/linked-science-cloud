...
const METHOD_DOCS = Object.freeze({
  bootstrap: {
    ...
    usage: "await (async () => { const { setupLinkedScience } = await import(`${process.cwd()}/lib/linked-science-runtime.mjs`); return setupLinkedScience({ nodeRepl: globalThis }); })()",
  ...
  'graphs.load': {
    ...
    signature: 'workspace.graphs.load({ name, kind, quads?, text?, format?, source? }) -> handle',
    kinds: [ ...GRAPH_KINDS ],
  ...
  orientation: {
    summary: 'Use the PEEK-aligned map for compact orientation, never as the ontology or result store.',
    methods: [ 'workspace.orientation.bootstrap()', 'current()', 'commit()', 'status()' ],
  },
  reset: {
...
function createWorkspace(runtime, context) {
  ...
  function updateOrientationForHandle(handle, record, role) {
  ...
  function loadGraph({ name, kind, quads, text, format = 'text/turtle', source = { kind: 'local-synthetic', id: name } } = {}) {
    ensureActive();
    ...
    updateOrientationForHandle(handle, resolve(handle), name);
    return handle;
  }
  ...
  async function runQuery({ sparql, sources, role } = {}, expectedType) {
    ...
    updateOrientationForHandle(handle, resolve(handle), role);
    return handle;
  }
  ...
  async function derive(handle, callback, { role } = {}) {
    ...
    updateOrientationForHandle(derivedHandle, resolve(derivedHandle), role);
  ...
  function orientationBootstrap({ checkpoint, maxItems = 30 } = {}) {
    ensureActive();
  ...
  function orientationCurrent({ maxBytes } = {}) {
    ensureActive();
  ...
  function orientationStatus() {
    ...
    const referenced = [ ...new Set(Object.values(context.orientationMap.sections).flatMap(entries => entries.flatMap(entry => entry.evidenceHandles))) ];
    return Object.freeze({
      status: 'ready', contextKey: context.key, epoch, entries: countOrientationEntries(context.orientationMap),
      handles: referenced.map(id => ({ id, status: registry.has(id) ? 'resident' : 'stale' })),
...
function createFacade({ nodeRepl, budgets, orientationCheckpoints = {} }) {
  const runtimeId = randomUUID();
  const contexts = new Map();
  const documentation = docsFunction();
  const runtime = { runtimeId, budgets: validateBudgets(budgets) };
  ...
  const facade = {
    ...
    reset({ contextKey } = {}) {
      ...
      return Object.freeze({ status: 'reset', contextKey, epoch: context.epoch, orientationRetained: Boolean(context.orientationMap), recoveryDocument: 'reset' });
    },
...
export async function setupLinkedScience({ nodeRepl, budgets, orientationCheckpoints } = {}) {
  ...
  const facade = createFacade({ nodeRepl, budgets, orientationCheckpoints });
