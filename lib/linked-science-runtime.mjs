...
const METHOD_DOCS = Object.freeze({
  ...
  budgets: {
    ...
    guidance: [ 'large RDF remains symbolic behind a graph handle', 'HEAD and Content-Length are advisory only', 'use direct remote subgraph queries for one-off slices', 'acquire and index once for repeated local queries' ],
  },
  compatibility: {
...
function budgetPlanes(budgets, traversal) {
  return freezeJson({
    ...
    residency: {
      ...
      semantics: 'operational in-memory safety; not a prompt or display limit',
    },
    projection: {
...
function createWorkspace(runtime, context) {
  ...
  async function runQuery({ sparql, sources, role } = {}, expectedType) {
    ...
    if (parsed.queryType !== 'ASK' && (!Number.isInteger(parsed.limit) || parsed.limit < 1 || parsed.limit > runtime.budgets.maxResultItems)) {
      throw queryValidationError(`Queries must include LIMIT 1-${runtime.budgets.maxResultItems}`, {
        field: 'sparql.limit',
        expected: { type: 'integer', minimum: 1, maximum: runtime.budgets.maxResultItems },
        action: `Add LIMIT between 1 and ${runtime.budgets.maxResultItems}; this changes only retained result admission and makes no live request.`,
      });
    }
