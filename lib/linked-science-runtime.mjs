...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    ...
    signature: "await workspace.traversal.query({ sparql, sources: [httpIri | httpsIri | { type: 'sparql', value: httpOrHttpsIri }], budgets?, role? }) -> retained native RDF/JS result handle",
    constraints: [ 'read-only SELECT/ASK/CONSTRUCT/DESCRIBE', 'credential-free HTTP/HTTPS source IRIs', 'no raw Fetch/Response authority', 'complete RDF documents use CONSTRUCT/queryQuads and bounded RDF/JS DatasetCore materialization', 'aggregate and per-exchange receipts', 'bounded duration, fan-out, concurrency, bytes, and items', 'no automatic PEEK or model-context promotion' ],
  },
  'results.derive': {
...
async function traversalCapabilities(traversal) {
  ...
  if (!capabilities || capabilities.kind !== 'linked-science-anonymous-read-mediator' || capabilities.version !== '3.0.0' ||
