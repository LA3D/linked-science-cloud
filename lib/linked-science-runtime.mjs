...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    summary: 'Execute a pre-grounded immutable scientific query plan through consumer-owned Communica while every request crosses a private parent mediator.',
    signature: 'await workspace.traversal.query(plan) -> retained native RDF/JS result handle',
    constraints: [ 'plan was created by workspace.grounding.plan after attestation', 'plan was enrolled before traversal.begin started the scored timer', 'read-only SELECT/ASK/CONSTRUCT/DESCRIBE', 'credential-free HTTP/HTTPS source IRIs', 'bounded identity-free content negotiation', 'no raw Fetch/Response authority', 'aggregate and per-exchange receipts', 'bounded duration, fan-out, concurrency, bytes, and items' ],
  },
  grounding: {
    ...
    methods: [ 'await workspace.grounding.begin({ target, budgets? })', 'await workspace.grounding.load({ name, document, source? })', 'workspace.grounding.use(handle)', 'await workspace.grounding.discover({ sparql, sources, negotiation?, role? })', 'await workspace.grounding.finish()', 'workspace.grounding.attest({ evidence, sourceChoices, graphChoices, predicateChoices })', 'workspace.grounding.plan({ sparql, sources, negotiation?, role? })', 'workspace.grounding.status()' ],
    constraints: [ 'schema, vocabulary, and dataset evidence are required', 'all evidence references must be resident typed handles', 'source, graph, and predicate choices cite evidence handles', 'discovery uses a distinct bounded mediator session', 'grounded evidence is registered as a bounded RLM context', 'planning completes before traversal.begin starts the scored session', 'grounded outputs may seed later resource grounding through the same typed contract', 'resource-specific facts remain declarative data' ],
  },
  'traversal.exploration': {
    summary: 'Attach one cumulative anonymous-read authority and receipt scope to this goal workspace across several agent decisions.',
    methods: [ 'await workspace.traversal.begin({ plans, budgets? })', 'await workspace.traversal.status()', 'await workspace.traversal.finish()', 'await workspace.traversal.abort(reason?)' ],
    constraints: [ 'one active scored exploration per goal workspace', 'all plans are grounded and enrolled before the scored timer begins', 'all requests, sources, bytes, duration, and result items consume the same cumulative budgets', 'raw or unplanned query calls are rejected' ],
  },
  'results.derive': {
...
export const LINKED_SCIENCE_API_SCHEMA = Object.freeze({
  ...
  workspace: {
    ...
    traversal: [ 'begin', 'query', 'status', 'finish', 'abort' ],
...
function createWorkspace(runtime, context) {
  ...
  function validateGroundingChoice(choice, name, evidenceIds) {
    if (!choice || typeof choice !== 'object' || typeof choice.term !== 'string' || choice.term.length < 1 || choice.term.length > 2_048) {
      throw runtimeError('LS_GROUNDING_CHOICE', 'grounding-attestation', `${name} choices require a bounded term`);
    }
    if (!Array.isArray(choice.evidenceHandles) || choice.evidenceHandles.length < 1 || choice.evidenceHandles.some(handle => !handle || !evidenceIds.has(handle.id))) {
      throw runtimeError('LS_GROUNDING_CHOICE', 'grounding-attestation', `${name} choices must cite resident attested evidence handles`);
    }
  ...
  async function beginGrounding({ target, budgets = {} } = {}) {
    ...
    if (typeof target !== 'string' || target.length < 1 || target.length > 512) throw runtimeError('LS_GROUNDING_TARGET', 'grounding-discovery', 'Grounding requires a bounded resource-neutral target description');
  ...
  async function loadGroundingEvidence({ name, document, source = { kind: 'declarative-resource-manifest', id: name } } = {}) {
    ...
    if (typeof name !== 'string' || !/^[a-z][a-z0-9-]{1,63}$/u.test(name)) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Evidence name must be a lowercase symbolic identifier');
    if (!document || typeof document !== 'object' || Array.isArray(document)) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Evidence document must be a declarative object');
    if (!source || ![ 'declarative-resource-manifest', 'local-documentation' ].includes(source.kind) || typeof source.id !== 'string' || source.id.length < 1) {
      throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-evidence', 'Evidence source must identify a declarative manifest or local documentation');
    }
    if (byteLength(document) > runtime.budgets.maxBytes) throw runtimeError('LS_GROUNDING_EVIDENCE_BOUND', 'grounding-evidence', 'Evidence document exceeds the runtime byte bound');
    const operationId = nextOperation();
  ...
  function attestGrounding({ evidence, sourceChoices, graphChoices, predicateChoices } = {}) {
    ...
    if (!Array.isArray(evidence) || evidence.length < 1) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Grounding attestation requires resident evidence handles');
    ...
    const attestedEvidence = evidence.map(item => {
      if (!item || typeof item !== 'object' || !Array.isArray(item.supports) || item.supports.length < 1 || typeof item.locator !== 'string' || item.locator.length < 1 || item.locator.length > 1_024) {
        throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Each evidence item requires a handle, supports list, and bounded locator');
      }
      const record = resolve(item.handle, new Set([ ...EVIDENCE_KINDS, ...RESULT_KINDS, ...GRAPH_KINDS ]));
      if (!grounding.evidenceHandles.has(item.handle.id)) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Evidence handle was not acquired during this grounding phase');
      const itemSupports = [ ...new Set(item.supports) ];
      if (itemSupports.some(value => ![ 'schema', 'vocabulary', 'dataset' ].includes(value))) throw runtimeError('LS_GROUNDING_EVIDENCE', 'grounding-attestation', 'Evidence supports must be schema, vocabulary, or dataset');
    ...
    for (const required of [ 'schema', 'vocabulary', 'dataset' ]) if (!supports.has(required)) {
      throw runtimeError('LS_GROUNDING_INCOMPLETE', 'grounding-attestation', `Grounding lacks ${required} evidence`);
    }
    if (!Array.isArray(sourceChoices) || sourceChoices.length < 1 || !Array.isArray(graphChoices) || graphChoices.length < 1 || !Array.isArray(predicateChoices) || predicateChoices.length < 1) {
      throw runtimeError('LS_GROUNDING_CHOICE', 'grounding-attestation', 'Grounding requires evidence-backed source, graph, and predicate choices');
    }
    ...
    if (byteLength(groundingContext) > runtime.budgets.maxBytes) throw runtimeError('LS_GROUNDING_CONTEXT_BOUND', 'grounding-attestation', 'Grounding context exceeds the runtime byte bound');
  ...
  function planGroundedQuery(options = {}) {
    ...
    try {
      if (typeof options.sparql !== 'string' || options.sparql.length < 1 || options.sparql.length > runtime.budgets.maxQueryChars) throw new Error('SPARQL is missing or exceeds the query character bound');
      if (!Array.isArray(options.sources) || options.sources.length < 1) throw new Error('At least one planned source is required');
    } catch (error) {
      throw wrapError(error, 'LS_QUERY_PLAN', 'query-planning');
    }
    ...
    if (byteLength(plannedContext) > runtime.budgets.maxBytes) throw runtimeError('LS_GROUNDING_CONTEXT_BOUND', 'query-planning', 'Grounding plan metadata exceeds the runtime byte bound');
    runtime.contextRegistry?.registerContext(grounding.contextId, plannedContext);
    grounding.context = plannedContext;
    queryPlans.set(id, { plan, options: plannedOptions, groundingSha256: grounding.attestationSha256, consumed: false });
    return plan;
  }
  ...
  async function beginExploration({ plans, budgets = {} } = {}) {
    ensureActive();
    ...
    if (!Array.isArray(plans) || plans.length < 1 || plans.length > 20) throw runtimeError('LS_QUERY_PLAN_REQUIRED', 'traversal-preflight', 'Enroll one to twenty immutable grounded plans before starting the scored timer');
    const enrolledPlans = plans.map(plan => {
      if (!plan || plan[QUERY_PLAN_BRAND] !== true || plan.kind !== 'linked-science-query-plan') throw runtimeError('LS_QUERY_PLAN_REQUIRED', 'traversal-preflight', 'An immutable query plan from this workspace is required');
      const record = queryPlans.get(plan.id);
      if (!record || record.plan !== plan || record.groundingSha256 !== grounding.attestationSha256 || record.consumed) throw runtimeError('LS_QUERY_PLAN_STALE', 'traversal-preflight', 'Query plan is unknown, stale, or already consumed');
      return plan.id;
    });
    const begun = await runtime.traversal.beginTraversal({ ...budgets, maxQueryChars: Math.min(budgets.maxQueryChars ?? runtime.budgets.maxQueryChars, runtime.budgets.maxQueryChars), maxResultItems: Math.min(budgets.maxResultItems ?? runtime.budgets.maxResultItems, runtime.budgets.maxResultItems) });
    exploration = { traversalId: begun.traversalId, startedAt: begun.startedAt, effectiveBudgets: begun.effectiveBudgets, resultItems: 0, operations: 0, enrolledPlans: new Set(enrolledPlans) };
    return freezeJson({ kind: 'linked-science-goal-exploration', status: 'active', protocolPhase: 'scored-traversal', contextKey: context.key, traversalId: exploration.traversalId, startedAt: exploration.startedAt, effectiveBudgets: exploration.effectiveBudgets, enrolledPlans, usage: { resultItems: 0, operations: 0 } });
  }
  ...
  async function explorationStatus() {
    ...
    return freezeJson({ kind: 'linked-science-goal-exploration', status: 'active', contextKey: context.key, traversalId: exploration.traversalId, usage: { ...receipt.usage, resultItems: exploration.resultItems, operations: exploration.operations }, budgets: receipt.budgets, navigation: navigationEvidence(receipt) });
  }
  ...
  async function finishExploration() {
    ...
    return freezeJson({ ...receipt, goalContextKey: context.key, resultItems: current.resultItems, operations: current.operations, navigation: navigationEvidence(receipt) });
  }
  ...
  async function executeMediatedQuery({ sparql, sources, negotiation = {}, role } = {}, scope, protocolPhase) {
    ensureActive();
    ...
    try {
      if (typeof sparql !== 'string' || sparql.length < 1 || sparql.length > runtime.budgets.maxQueryChars) throw new Error('SPARQL is missing or exceeds the query character bound');
      parsed = new SparqlParser().parse(sparql);
      if (parsed.type !== 'query' || !QUERY_TYPES.has(parsed.queryType)) throw new Error('Only SELECT, ASK, CONSTRUCT, and DESCRIBE are supported');
      if (!Array.isArray(sources) || sources.length < 1 || sources.length > 20) throw new Error('One to twenty anonymous HTTP/HTTPS source IRIs are required');
      const defaultNegotiation = normalizeNegotiation(negotiation);
      sourceNegotiations = new Map();
      mediatedSources = sources.map(source => {
        ...
        if (descriptor && (typeof descriptor.value !== 'string' || Object.keys(descriptor).some(key => ![ 'type', 'value', 'negotiation' ].includes(key)) || (descriptor.type !== undefined && descriptor.type !== 'sparql'))) {
          throw new Error("Traversal source descriptors may contain only { value, negotiation? } or { type: 'sparql', value }");
        }
        if (descriptor?.type === 'sparql' && descriptor.negotiation !== undefined) throw new Error('Document profile negotiation cannot be applied to a SPARQL service source');
        const url = new URL(descriptor?.value ?? source);
        if (![ 'http:', 'https:' ].includes(url.protocol) || url.username || url.password) throw new Error('Traversal sources must be credential-free HTTP or HTTPS IRIs');
      ...
      if ([ ...defaultNegotiation ].length > 0 && sourceNegotiations.size === 0) {
        throw new Error('Top-level document negotiation requires at least one document source; SPARQL services do not inherit document negotiation headers');
      }
    } catch (error) {
      throw wrapError(error, 'LS_TRAVERSAL_PREFLIGHT', 'traversal-preflight', { operationId });
    }
    let traversalId;
    try {
      ...
      throw wrapError(error, 'LS_TRAVERSAL_QUERY', 'traversal-query', { operationId, traversalId, traversalReceipt, navigation });
    }
  }
  ...
  async function traverseQuery(plan) {
    ...
    if (!plan || plan[QUERY_PLAN_BRAND] !== true || plan.kind !== 'linked-science-query-plan') throw runtimeError('LS_QUERY_PLAN_REQUIRED', 'traversal-preflight', 'traversal.query accepts only an immutable grounded plan');
    const record = queryPlans.get(plan.id);
    if (!record || record.plan !== plan || !exploration.enrolledPlans.has(plan.id) || record.consumed) throw runtimeError('LS_QUERY_PLAN_STALE', 'traversal-preflight', 'Query plan was not enrolled or is already consumed');
    record.consumed = true;
    return executeMediatedQuery(record.options, exploration, 'scored-scientific-query');
  }
  ...
  const workspace = {
    ...
    traversal: Object.freeze({ begin: beginExploration, query: traverseQuery, status: explorationStatus, finish: finishExploration, abort: abortExploration }),
