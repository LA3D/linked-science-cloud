...
const METHOD_DOCS = Object.freeze({
  ...
  'query.select': {
    ...
    constraints: [ 'sources accept graph handles and quad-result handles', 'broker-stored quad results stream into later local queries', 'SPARQL LIMIT is optional caller semantics and is never imposed by the harness', 'successful handles are complete; residency exhaustion fails without a partial handle', 'SERVICE rejected', 'no network sources' ],
  },
  'query.run': {
    summary: 'Run complete-or-fail local SELECT, ASK, CONSTRUCT, or DESCRIBE through Communica; the clean-room broker spools graph results out of core.',
    ...
    constraints: [ 'SPARQL solution modifiers remain query semantics', 'DESCRIBE uses the declared outgoing-subject-triples policy', 'model-visible projection bounds do not change resident results', 'broker-backed CONSTRUCT/DESCRIBE results are bounded by storage bytes rather than result-item or prompt limits' ],
  },
  'traversal.query': {
  ...
  'rdf.dataset': {
    summary: 'Return a cloned native N3 RDF/JS DatasetCore for ordinary JavaScript or installed-library composition inside the REPL; broker-stored results remain streaming query sources instead.',
  ...
  budgets: {
    ...
    planes: {
      ...
      residency: [ 'resource body bytes', 'per-graph quads', 'workspace graph quads', 'kernel-resident result items', 'broker-stored quad-result bytes' ],
...
function serializeTerm(term) {
  ...
  return {
    ...
    value: term.value,
    ...(term.termType === 'Literal' ? { language: term.language ?? '', datatype: term.datatype?.value ?? 'http://www.w3.org/2001/XMLSchema#string' } : {}),
  };
}
...
function serializeQuad(quad) {
  const copied = copyQuad(quad);
  return {
    subject: serializeTerm(copied.subject),
    predicate: serializeTerm(copied.predicate),
    object: serializeTerm(copied.object),
    graph: serializeTerm(copied.graph),
  };
}
...
function createStoredQuadSource(stored, resultStorage, pageSize) {
  const matches = (term, pattern) => pattern == null || term.equals(pattern);
  return Object.freeze({
    match(subject, predicate, object, graph) {
      return Readable.from((async function* () {
        let offset = 0;
        while (offset < stored.count) {
          const page = await resultStorage.page(stored.storageId, { offset, limit: pageSize });
          if (!Array.isArray(page.items) || page.items.length === 0) break;
          for (const serialized of page.items) {
            const quad = deserializeQuad(serialized);
            if (matches(quad.subject, subject) && matches(quad.predicate, predicate) && matches(quad.object, object) && matches(quad.graph, graph)) yield quad;
          }
...
async function resultStorageCapabilities(resultStorage) {
  ...
  for (const method of [ 'capabilities', 'begin', 'append', 'commit', 'page', 'abort' ]) {
  ...
  if (!capabilities || capabilities.kind !== 'linked-science-result-spool' || capabilities.version !== '1.0.0' || capabilities.owner !== 'cleanroom-broker' ||
    capabilities.completeBeforePublication !== true || !Array.isArray(capabilities.resultKinds) || !capabilities.resultKinds.includes('quads') ||
...
async function collectQuadResult(stream, { resultStorage, inMemoryMaximum, fallbackMaximum, code, stage }) {
  ...
  const flush = async () => {
    ...
    await resultStorage.append(begun.storageId, batch);
    batch = [];
    batchBytes = 2;
  };
  const queue = async serialized => {
    const itemBytes = byteLength(serialized) + 1;
    if (itemBytes + 2 > begun.maxAppendBytes) {
      throw runtimeError('LS_RESULT_STORAGE_ITEM_LIMIT', 'result-storage', `One RDF quad exceeds the broker append ceiling of ${begun.maxAppendBytes} bytes`, {
        receipt: { status: 'failed', stage: 'result-storage', code: 'LS_RESULT_STORAGE_ITEM_LIMIT', maxAppendBytes: begun.maxAppendBytes },
        recoveryDocument: 'budgets',
      });
    }
    if (batch.length > 0 && (batchBytes + itemBytes > begun.maxAppendBytes || batch.length >= begun.maxAppendItems)) await flush();
    batch.push(serialized);
    batchBytes += itemBytes;
  };
  try {
    for await (const item of stream) {
      const copied = copyQuad(item);
      if (!begun) {
        memory.addQuad(copied);
        if (memory.size <= inMemoryMaximum) continue;
        begun = await resultStorage.begin({ kind: 'quads' });
        for (const retained of memory) await queue(serializeQuad(retained));
        memory = undefined;
        continue;
      }
      await queue(serializeQuad(copied));
    }
    if (!begun) return memory;
    await flush();
    const committed = await resultStorage.commit(begun.storageId);
    if (!isStoredResult(committed)) throw runtimeError('LS_RESULT_STORAGE_COMMIT', 'result-storage', 'Broker result storage returned an invalid completion descriptor');
    return freezeJson(committed);
  } catch (error) {
    stream.destroy?.();
    if (begun) try { await resultStorage.abort(begun.storageId); } catch {}
    if (error?.code === 'RESULT_SPOOL_RESULT_LIMIT' || error?.code === 'RESULT_SPOOL_TOTAL_LIMIT') {
      ...
      throw runtimeError(
        perResult ? 'LS_QUERY_RESULT_STORAGE_BOUND' : 'LS_RESULT_STORAGE_EXHAUSTED',
        'result-storage',
        `${error.message}; no partial result handle was retained`,
        {
          receipt: {
            status: 'failed',
            stage: 'result-storage',
            code: perResult ? 'LS_QUERY_RESULT_STORAGE_BOUND' : 'LS_RESULT_STORAGE_EXHAUSTED',
            repair,
          },
          recoveryDocument: 'budgets',
          retryable: true,
...
function budgetPlanes(budgets, traversal, resultStorage) {
  return freezeJson({
    execution: {
    ...
    residency: {
      ...
      brokerStoredQuadResults: resultStorage ? {
        available: true,
        backend: resultStorage.backend,
        maxResultBytes: resultStorage.maxResultBytes,
        maxTotalBytes: resultStorage.maxTotalBytes,
        semantics: 'out-of-core graph-result capacity; never a SPARQL solution modifier or prompt bound',
      } : { available: false },
...
function createWorkspace(runtime, context) {
  ...
  const querySourceForQuads = value => isStoredResult(value)
    ? createStoredQuadSource(value, runtime.resultStorage, runtime.resultStorageCapabilities.maxPageItems)
    : value;
  ...
  async function runQuery({ sparql, sources, role } = {}, expectedType) {
    ...
    const hasStoredSource = sourceRecords.some(record => isStoredResult(record.value));
    const querySources = hasStoredSource
      ? sourceRecords.map(record => record.querySource)
      : [ new Store(sourceRecords.flatMap(record => GRAPH_KINDS.has(record.kind) ? record.value : [ ...record.value ])) ];
    let kind;
    let value;
    let execution;
    try {
      ...
      if (parsed.queryType === 'SELECT') {
        kind = 'bindings';
        value = normalizeBindings(await collectBounded(await engine.queryBindings(execution.sparql, { sources: querySources }), runtime.budgets.maxResultItems, 'LS_QUERY_RESULT_RESIDENCY_BOUND', 'query-residency'));
        ...
        value = await collectQuadResult(stream, {
          resultStorage: runtime.resultStorage,
  ...
  async function traverseQuery({ sparql, sources, negotiation = {}, role, evidence = [], budgets = {} } = {}) {
    ...
    try {
      ...
      const resultItems = kind === 'quads' ? resultCount({ kind, value }) : kind === 'boolean' ? 1 : value.length;
      ...
      const payloadSha256 = kind === 'quads' ? (isStoredResult(value) ? value.sha256 : quadFingerprint([ ...value ])) : compactHash(value);
      ...
      if (pendingStoredResult) {
        try { await runtime.resultStorage.abort(pendingStoredResult.storageId); } catch {}
      }
  ...
  function resultPage(handle, { offset = 0, limit, columns, maxCells, maxBytes } = {}) {
    ...
    if (isStoredResult(record.value)) {
      return runtime.resultStorage.page(record.value.storageId, { offset: boundedOffset, limit: rowLimit })
        .then(page => build(page.items.map(deserializeQuad)));
    }
...
function createFacade({ budgets, orientationCheckpoints = {}, peek, environment, traversal, traversalCapabilityReceipt, resultStorage, resultStorageCapabilityReceipt }) {
  ...
  const runtime = {
    ...
    budgetPlanes: budgetPlanes(validatedBudgets, traversalCapabilityReceipt, resultStorageCapabilityReceipt),
    peek,
    traversal,
