import { createWorkspaceReasoning } from './linked-science-reasoning.mjs';
...
const METHOD_DOCS = Object.freeze({
  ...
  'reasoning.run': {
    summary: 'Run bounded deterministic N3 rules over one retained default-graph RDF source; preserve assertions and retain a separate inferred graph.',
    signature: "await workspace.reasoning.run({ sources: [handle], rules: { id, version, text }, graphPolicy: 'default-graph-only', proof?, limits? }) -> { derived, proof, report }",
    constraints: [ 'requires the pinned host Eyeron adapter', 'named graphs rejected; explicitly construct the intended input first', 'complete-or-fail; source release/reset invalidates derived reads', 'no embedded language model or implicit printing', 'explicit rules are caller-supplied premises, not automatically authoritative ontology semantics' ],
  },
  'reasoning.describe': {
    summary: 'Inspect compact completion, input/rule/engine provenance and validity for a live reasoning run.',
    signature: 'workspace.reasoning.describe(run) -> report',
  },
  'reasoning.explain': {
    summary: 'Read a bounded raw N3 proof excerpt; not a verified proof or selected-conclusion explanation.',
    signature: 'workspace.reasoning.explain(run, { maxBytes? }) -> proof excerpt',
  },
  'reasoning.capabilities': {
    summary: 'Inspect pinned engine availability and execution limits without running inference.',
    signature: 'workspace.reasoning.capabilities() -> capability receipt',
  },
  'rdf.source': {
...
export const LINKED_SCIENCE_API_SCHEMA = Object.freeze({
  ...
  workspace: {
    ...
    schema: [ 'search' ],
    query: [ 'run', 'select' ],
    reasoning: [ 'capabilities', 'run', 'describe', 'explain' ],
    evidence: [ 'load' ],
...
function createWorkspace(runtime, context) {
  ...
  const reasoningDependencies = new Map();
  ...
  function resolve(handle, allowedKinds, skipDependencies = false) {
    ensureActive();
    ...
    if (!skipDependencies) for (const dependency of reasoningDependencies.get(handle.id) ?? []) resolve(dependency);
    return record;
  }

  async function release(value) {
    const handle = value?.handle ?? value;
    const record = resolve(handle, undefined, true);
    registry.delete(handle.id);
    reasoningDependencies.delete(handle.id);
    released.add(handle);
  ...
  function sourceReference(handle, record, role) {
    ...
    return {
      ...
      ...(record.provenance.reasoning ? { reasoning: {
        complete: true, semantics: record.provenance.reasoning.semantics,
        rulesSha256: record.provenance.reasoning.rules.sha256,
        inputSha256: record.provenance.reasoning.inputSha256,
        graphPolicy: record.provenance.reasoning.graphPolicy,
        proofAvailable: record.provenance.reasoning.proof.available,
        validity: 'revalidate-resident-dependencies',
      } } : {}),
    ...
    try {
      return await orientationSerial(async () => {
        ...
        const displayed = entries.map(entry => {
          const value = envelope(entry);
          if (value.kind !== 'linked-science-semantic-entry') {
            let validity;
            if (value.reasoning) {
              try { if (value.contextKey !== context.key || value.handleEpoch !== epoch) throw new Error(); resolve(registry.get(value.handle)?.handle); validity = 'valid'; }
              catch { validity = 'stale-or-released'; }
            }
            return { id: entry.id, section: entry.section, ...value, ...(validity ? { reasoning: { ...value.reasoning, validity } } : {}) };
          }
  ...
  const reasoning = createWorkspaceReasoning({
    adapter: runtime.reasoning, capabilities: runtime.reasoningCapabilities,
    source: rdfSource, resolve, release, ensureActive, operation: nextOperation,
    maxBytes: runtime.budgets.maxBytes,
    depend: (handle, sources) => reasoningDependencies.set(handle.id, [...sources]),
    orient: handle => updateOrientationForHandle(handle, resolve(handle), "inferred-evidence"),
    retainGraph: (quads, report) => retainGraph('inferred-graph', 'reasoning-derived', quads, {
      provenance: { operationId: report.operationId, localOnly: true, completion: { complete: true }, reasoning: report },
      lineage: { kind: 'deterministic-inference', operationId: report.operationId, sources: report.inputs.map(x => x.id) },
      fingerprints: [quadFingerprint(quads)],
    }),
    retainProof: (text, report) => retain('evidence', 'reasoning-proof', Object.freeze({ format: 'N3', text, verified: false }), {
      provenance: { operationId: report.operationId, localOnly: true, sha256: report.proof.sha256 },
      lineage: { kind: 'reasoning-proof', operationId: report.operationId }, fingerprints: [report.proof.sha256],
    }),
  });
...
function createFacade({ budgets, orientationCheckpoints = {}, peek, environment, traversal, traversalCapabilityReceipt, resultStorage, resultStorageCapabilityReceipt, reasoning, reasoningCapabilityReceipt }) {
  ...
  const runtime = {
    ...
    reasoning, reasoningCapabilities: reasoningCapabilityReceipt,
  };
  ...
  const facade = {
    ...
    capabilities() {
      return Object.freeze({
        ...
        reasoning: runtime.reasoningCapabilities,
...
export async function setupLinkedScience({ nodeRepl, budgets, orientationCheckpoints, peek, environment, traversal, resultStorage, reasoning } = {}) {
  ...
  const reasoningCapabilityReceipt = reasoning ? freezeJson(jsonClone(await reasoning.capabilities())) : Object.freeze({ available: false });
  const facade = createFacade({ budgets, orientationCheckpoints, peek, environment, traversal, traversalCapabilityReceipt, resultStorage, resultStorageCapabilityReceipt, reasoning, reasoningCapabilityReceipt });
