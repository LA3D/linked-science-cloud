...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    summary: 'Use consumer-owned Communica and native RDF/JS results over dynamically selected anonymous HTTP/HTTPS Linked Data sources while every request crosses a private parent mediator.',
    signature: "await workspace.traversal.query({ sparql, sources: [httpIri | httpsIri | { value: httpOrHttpsIri, negotiation? } | { type: 'sparql', value: httpOrHttpsIri }], negotiation?: { accept?, acceptProfile?, prefer? }, budgets?, role? }) -> retained native RDF/JS result handle",
    constraints: [ 'read-only SELECT/ASK/CONSTRUCT/DESCRIBE', 'credential-free HTTP/HTTPS source IRIs', 'bounded identity-free content negotiation', 'no raw Fetch/Response authority', 'complete RDF documents use CONSTRUCT/queryQuads and bounded RDF/JS DatasetCore materialization', 'aggregate and per-exchange receipts with bounded untrusted navigation candidates', 'bounded duration, fan-out, concurrency, bytes, and items', 'no automatic link following, PEEK, memory, or model-context promotion' ],
  },
  'traversal.exploration': {
    ...
    methods: [ 'await workspace.traversal.begin({ budgets })', 'await workspace.traversal.status()', 'await workspace.traversal.finish()', 'await workspace.traversal.abort(reason?)' ],
    constraints: [ 'one active exploration per goal workspace', 'all requests, sources, bytes, duration, and result items consume the same cumulative budgets', 'destinations remain dynamically discoverable', 'simple query calls remain one-shot when no exploration is active' ],
  },
  'results.derive': {
...
function createWorkspace(runtime, context) {
  ...
  async function beginExploration({ budgets = {} } = {}) {
    ensureActive();
    ...
    exploration = { traversalId: begun.traversalId, startedAt: begun.startedAt, effectiveBudgets: begun.effectiveBudgets, resultItems: 0, operations: 0 };
    return freezeJson({ kind: 'linked-science-goal-exploration', status: 'active', contextKey: context.key, traversalId: exploration.traversalId, startedAt: exploration.startedAt, effectiveBudgets: exploration.effectiveBudgets, usage: { resultItems: 0, operations: 0 } });
  }
  ...
  async function traverseQuery({ sparql, sources, negotiation = {}, role, budgets = {} } = {}) {
    ensureActive();
    ...
    try {
      ...
      if (exploration && Object.keys(budgets).length) throw new Error('Budgets belong to traversal.begin while a goal exploration is active');
    } catch (error) {
    ...
    try {
      ...
      if (exploration) {
        traversalId = exploration.traversalId;
        effectiveBudgets = exploration.effectiveBudgets;
      } else {
        oneShot = true;
        const begun = await runtime.traversal.beginTraversal({ ...budgets, maxQueryChars: Math.min(budgets.maxQueryChars ?? runtime.budgets.maxQueryChars, runtime.budgets.maxQueryChars), maxResultItems: Math.min(budgets.maxResultItems ?? runtime.budgets.maxResultItems, runtime.budgets.maxResultItems) });
      ...
      const remainingResultItems = exploration ? effectiveBudgets.maxResultItems - exploration.resultItems : effectiveBudgets.maxResultItems;
      ...
      if (exploration) {
        exploration.resultItems += resultItems;
        exploration.operations += 1;
      }
      const receipt = oneShot ? await runtime.traversal.finishTraversal(traversalId) : await runtime.traversal.snapshotTraversal(traversalId);
      if (receipt?.kind !== 'linked-science-traversal-receipt' || ![ 'complete', 'active' ].includes(receipt.status) || receipt.traversalId !== traversalId) {
      ...
      if (traversalId) {
        ...
        if (oneShot) try { await runtime.traversal.abortTraversal(traversalId, error.code ?? 'query-failed'); } catch {}
      }
...
function createFacade({ nodeRepl, budgets, orientationCheckpoints = {}, peek, environment, traversal, traversalCapabilityReceipt }) {
...
export async function setupLinkedScience({ nodeRepl, budgets, orientationCheckpoints, peek, environment, traversal } = {}) {
  ...
  const facade = createFacade({ nodeRepl, budgets, orientationCheckpoints, peek, environment, traversal, traversalCapabilityReceipt });
