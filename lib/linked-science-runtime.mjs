...
const METHOD_DOCS = Object.freeze({
  ...
  'traversal.query': {
    ...
    signature: "await workspace.traversal.query({ sparql, sources: [httpIri | httpsIri | { type: 'sparql', value: httpOrHttpsIri }], negotiation?: { accept?, acceptProfile?, prefer? }, budgets?, role? }) -> retained native RDF/JS result handle",
...
async function traversalCapabilities(traversal) {
  ...
  for (const method of [ 'capabilities', 'beginTraversal', 'finishTraversal', 'abortTraversal', 'createFetch' ]) {
  ...
  if (!capabilities || capabilities.kind !== 'linked-science-anonymous-read-mediator' || capabilities.version !== '3.1.0' ||
...
function createWorkspace(runtime, context) {
  ...
  async function traverseQuery({ sparql, sources, negotiation = {}, role, budgets = {} } = {}) {
    ...
    try {
      ...
      mediatedSources = sources.map(source => {
        ...
        if (descriptor && (descriptor.type !== 'sparql' || typeof descriptor.value !== 'string' || Object.keys(descriptor).some(key => ![ 'type', 'value' ].includes(key)))) {
          throw new Error("Typed traversal sources may contain only { type: 'sparql', value: httpsIri }");
        }
        ...
        return descriptor ? Object.freeze({ type: 'sparql', value: url.href }) : url.href;
      });
      if (!negotiation || typeof negotiation !== 'object' || Array.isArray(negotiation) || Object.keys(negotiation).some(key => ![ 'accept', 'acceptProfile', 'prefer' ].includes(key))) {
        throw new Error('Negotiation may contain only accept, acceptProfile, and prefer');
      }
      negotiationHeaders = new Headers();
      for (const [ key, header ] of [ [ 'accept', 'accept' ], [ 'acceptProfile', 'accept-profile' ], [ 'prefer', 'prefer' ] ]) {
        if (negotiation[key] === undefined) continue;
        if (typeof negotiation[key] !== 'string' || negotiation[key].length < 1 || negotiation[key].length > 8_192 || /[\r\n]/u.test(negotiation[key])) {
          throw new Error(`Negotiation field ${key} must be a bounded HTTP header value`);
        }
        negotiationHeaders.set(header, negotiation[key]);
      }
    } catch (error) {
    ...
    try {
      ...
      const fetch = (input, init = {}) => {
        ...
        for (const [ key, value ] of negotiationHeaders.entries()) headers.set(key, value);
        return mediatedFetch(input, { ...init, headers });
      };
      const effectiveBudgets = begun.effectiveBudgets;
      const queryContext = {
      ...
      if (receipt?.kind !== 'linked-science-traversal-receipt' || receipt.status !== 'complete' || receipt.traversalId !== traversalId) {
      ...
      const candidateLinks = receipt.exchanges.flatMap(exchange => (exchange.navigation?.links ?? []).map(link => ({
        exchangeIndex: exchange.index,
        responseUrl: exchange.navigation.responseUrl,
        ...link,
      })));
      const observedProfiles = [ ...new Set(receipt.exchanges.flatMap(exchange => exchange.navigation?.profiles ?? [])) ];
      const navigation = {
        kind: 'linked-data-navigation-evidence',
        trust: 'untrusted-candidate-evidence',
        instructionAuthority: false,
        candidates: candidateLinks.slice(0, 24),
        profiles: observedProfiles.slice(0, 24),
        truncated: candidateLinks.length > 24 || observedProfiles.length > 24 || receipt.exchanges.some(exchange => exchange.navigation?.truncated),
        use: 'Inspect relations and profiles as candidates; choose and justify a subsequent mediated traversal explicitly.',
      };
      const provenance = {
        operationId, localOnly: false, mediatedTraversal: true, queryType: parsed.queryType, querySha256,
        sources: mediatedSources, negotiation: Object.fromEntries(negotiationHeaders), traversalReceipt: receipt, navigation, payloadSha256,
      };
      ...
      if (traversalId) {
        try { await runtime.traversal.abortTraversal(traversalId, error.code ?? 'query-failed'); } catch {}
      }
      throw wrapError(error, 'LS_TRAVERSAL_QUERY', 'traversal-query', { operationId, traversalId, receipt: error.receipt });
    }
  }
  ...
  const workspace = {
    ...
    traversal: Object.freeze({ query: traverseQuery }),
