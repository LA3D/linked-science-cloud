...
function applyHeapResidency(budgets) {
  ...
  return {
    ...
    basis: Object.freeze({
      ...
      configured,
      heap,
      note: 'Resident-graph quotas never exceed what the kernel heap can hold; a live headroom check also guards each large retention or copy.',
    }),
  };
}
...
function createWorkspace(runtime, context) {
  ...
  const querySourceForQuads = value => isStoredResult(value)
    ? createStoredQuadSource(value, runtime.resultStorage, runtime.resultStorageCapabilities.maxMatchItems ?? runtime.resultStorageCapabilities.maxPageItems)
    : value;
  ...
  async function updateOrientationForHandle(handle, record, role) {
    if (runtime.peek) {
      ...
      const text = JSON.stringify({
        ...
        type: record.kind,
        count: RESULT_KINDS.has(record.kind) ? resultCount(record) : Array.isArray(record.value) ? record.value.length : 1,
      });
    ...
    context.orientationMap = recordOrientation(context.orientationMap, {
      ...
      value: { handle: handle.id, role: role ?? handle.label, kind: record.kind, count: RESULT_KINDS.has(record.kind) ? resultCount(record) : Array.isArray(record.value) ? record.value.length : 1 },
  ...
  async function retainRdf({ name, dataset, quads, kind = 'instance-data', role } = {}) {
    ...
    ensureHeapHeadroom(values.length * RESIDENT_QUAD_BYTES_ESTIMATE, { stage: 'rdf-retain', operation: `copying and indexing ${values.length} in-kernel quads` });
  ...
  function resultPage(handle, { offset = 0, limit, columns, maxCells, maxBytes } = {}) {
    ...
    if (isStoredResult(record.value)) {
      return runtime.resultStorage.page(record.value.storageId, { offset: boundedOffset, limit: rowLimit })
        .then(page => build(page.items.map(record.kind === 'bindings' ? deserializeBindingRow : deserializeQuad)));
    }
