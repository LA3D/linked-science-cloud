/** @deprecated Historical exact-document experiment helper; not a production transport or ontology gate. */
...
export function createDocumentationClient({ fetchImpl = globalThis.fetch } = {}) {
  return Object.freeze({
    async fetch(profileName = 'uniprotRdfSchema') {
      const profile = getDocumentationProfile(profileName);
      if (profile.sources.length !== 1) {
        throw new Error(`Profile ${profileName} requires an explicit source; it declares ${profile.sources.length} sources`);
      }
      return createGuardedDocumentationFetch({ profile, fetchImpl }).fetchDocument(profile.sources[0]);
    },
  });
}
...
export function createGuardedDocumentationFetch({ profile = getDocumentationProfile(), fetchImpl = globalThis.fetch } = {}) {
  ...
  const fetchDocument = async(source) => {
    ...
    try {
      ...
      const receipt = Object.freeze({ ...entry, status: response.status, ok: response.ok, contentType, byteLength: bytes.length, sha256: createHash('sha256').update(bytes).digest('hex') });
