...
export function assertSymbolicValue(value, depth = 0) {
  if (depth > 4) throw new Error('Orientation values may nest at most four levels');
  if (value === null || typeof value === 'boolean' || typeof value === 'number') return true;
  if (typeof value === 'string') {
    // An identifier such as "service-schema" or a source URL is not SPARQL.
    if (value.length === 0 || value.length > 1_024 || (RAW_QUERY_MARKERS.test(value) && /[\s<>{}]/u.test(value))) throw new Error('Orientation strings must be compact and exclude raw query text');
    return true;
  }
...
// The experimental facade uses the same compact JSON envelope as broker PEEK.
function assertEntryValue(kind, value) {
  if (kind !== 'context-envelope') return assertSymbolicValue(value);
  if (!value || Object.keys(value).join() !== 'json' || typeof value.json !== 'string' || Buffer.byteLength(value.json) > 8192) throw new Error('Invalid context envelope');
  const parsed = JSON.parse(value.json);
  if (!['linked-science-context-reference', 'linked-science-semantic-entry'].includes(parsed?.kind)) throw new Error('Invalid context envelope kind');
}

export function deleteOrientation(contextMap, key) {
  const map = clone(contextMap);
  for (const section of ORIENTATION_SECTIONS) map.sections[section] = map.sections[section].filter(entry => entry.key !== key);
...
export function assertOrientationMap(map) {
  ...
  for (const section of ORIENTATION_SECTIONS) {
    ...
    for (const item of map.sections[section]) {
      ...
      assertEntryValue(item.kind, item.value);
    }
  }
  return true;
}
