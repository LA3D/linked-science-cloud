...
export async function inspectLinkedScienceBootstrap({
  ...
  if (cleanroom.linkedScienceBroker !== undefined) {
    for (const method of [ 'capabilities', 'acquire', 'query' ]) requireMethod(cleanroom.linkedScienceBroker, method);
  }
  ...
  return Object.freeze({
    ...
    networkBroker: Object.freeze({ available: cleanroom.linkedScienceBroker !== undefined, owner: 'cleanroom-broker' }),
  });
}
...
export async function bootstrapLinkedScience({
  ...
  const facade = await setupLinkedScience({ nodeRepl: host, peek: cleanroom.peek, environment, broker: cleanroom.linkedScienceBroker });
