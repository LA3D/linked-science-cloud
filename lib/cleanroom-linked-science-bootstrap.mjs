...
export async function inspectLinkedScienceBootstrap({
  ...
  if (cleanroom.linkedScienceTraversal !== undefined) {
    for (const method of [ 'capabilities', 'beginTraversal', 'request', 'finishTraversal', 'abortTraversal', 'createFetch' ]) requireMethod(cleanroom.linkedScienceTraversal, method);
  }
  ...
  return Object.freeze({
    ...
    traversalMediator: Object.freeze({ available: cleanroom.linkedScienceTraversal !== undefined, owner: 'cleanroom-broker' }),
  });
}
...
export async function bootstrapLinkedScience({
  ...
  const facade = await setupLinkedScience({ nodeRepl: host, peek: cleanroom.peek, environment, traversal: cleanroom.linkedScienceTraversal });
