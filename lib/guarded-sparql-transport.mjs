...
function includesService(value) {
  if (!value || typeof value !== 'object') return false;
  if (Array.isArray(value)) return value.some(includesService);
  if (String(value.type ?? '').toLowerCase() === 'service') return true;
  return Object.values(value).some(includesService);
}
...
export function validateReadQuery(query, profile = getEndpointProfile()) {
  ...
  if (parsed.type !== 'query' || !READ_QUERY_TYPES.has(parsed.queryType)) {
    throw new Error('Only SELECT, ASK, CONSTRUCT, and DESCRIBE queries are permitted');
  }
  if (includesService(parsed)) throw new Error('SERVICE clauses are not permitted');
  ...
  return Object.freeze({ queryType: parsed.queryType, limit: parsed.limit ?? 1 });
}

function exactEndpoint(url, profile) {
  const expected = new URL(profile.endpoint);
