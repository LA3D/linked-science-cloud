...
const names = ['episode', 'objective', 'evidence', 'outcome'];
...
// four checked-in contracts, not a general JSON Schema implementation.
...
export function validateEpisode(input) {
  ...
  for (const objective of objectives.values()) {
    ...
    if (!event || event.kind !== 'userMessage' || !event.objectiveIds.includes(objective.id)) fail('objective must link an observable user message');
