...
export async function inspectEpisodeEvidence(input, { root } = {}) {
  ...
  if (!hostArtifact) missing.push('observable-events-artifact');
    ...
    if (episode.capture.kind === 'direct-runtime') for (const objective of episode.objectives) {
      const event = episode.events.find(item => item.id === objective.eventId);
      if (hostItems.get(`${event.turnId}/${event.itemId}`)?.result?.structuredContent?.question !== objective.statement) throw new Error('Runtime objective declaration does not match observed tool output');
    }
  }
