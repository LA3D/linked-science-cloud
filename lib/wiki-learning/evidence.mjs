...
async function artifact(root, descriptor) {
  let bytes;
...
export async function inspectEpisodeEvidence(input, { root } = {}) {
  ...
  const missing = [];
  const hostArtifact = await artifact(root, episode.host.observableEvents);
  let hostItems = null;
  ...
  for (const entry of episode.evidence) {
    const content = entry.artifact ? await artifact(root, entry.artifact) : null;
