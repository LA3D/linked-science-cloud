...
const WORKER_FORMAT = 'linked-science-competency-worker/v1';
...
const UNREVIEWED_PROFILE = /(?:pending|placeholder|unreviewed)/iu;
...
function validateWorkerCase(item, index) {
  ...
  for (const [ resourceIndex, resource ] of item.resources.entries()) {
    ...
    requireString(resource.profile, `${resourceLabel}.profile`, { maximum: 120 });
...
export function validateWorkerManifest(input, { requireDispatchable = false } = {}) {
  ...
  if (manifest.status === 'ready' && manifest.cases.some(item => item.resources.some(resource => UNREVIEWED_PROFILE.test(resource.profile)))) {
    throw new Error('ready worker manifest cannot contain pending or unreviewed profile names');
  }
  if (requireDispatchable && manifest.cases.some(item => item.resources.some(resource => resource.authorizationRequired !== true))) {
    throw new Error('dispatchable worker cases must retain explicit per-profile approval gates');
  }
