...
Local consumer-owned Communica receives agent-selected HTTP/HTTPS RDF sources and may execute `SERVICE` federation, while every request crosses a module-private parent mediator. The agent receives neither Fetch nor the bridge. The mediator strips ambient identity, accepts only read operations, and enforces request, fan-out, concurrency, duration, byte, and item ceilings. These are operational controls: the runtime never imposes a SPARQL `LIMIT`, and it retains a result handle only after the native stream completes. Exceeding a ceiling is a failed attempt with no partial handle. Transport libraries make no hidden retry.
