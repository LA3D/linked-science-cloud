...
The [wiki-memory implementation plan](docs/tasks/wiki-memory-continual-learning.md) specifies an independent repository-owned Science Cloud wiki, reuse of Codex history, skill validation and release/rollback. It is separate from the user's Obsidian wikimemory. The authorized phase-0 contracts, frozen skill baseline and synthetic receipt fixture are implemented. Later imports, runtime scientific memory and learning evaluations remain proposed and require their own authorization.
