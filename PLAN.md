...
- owner (`caller-observed` or `package-child`), exact first/last compacted source event IDs and event digests, previous compaction record, first retained event ID, compacted session identity and active compaction pointer, and retention pins;
...
Branch summarization is separate from compaction. `CompactionManager.summarizeBranch({sessionId, sourceHead, targetHead, commonAncestor, budget})` creates a distinct `branch-summary@1` when navigation leaves a branch. The durable record contains source and target session/branch IDs and head event IDs/digests; common-ancestor event ID/digest; one or more exact summarized event ranges; provider invocation ID and `resource-fact@1` links under role `branch-summary`; immutable summary artifact ID/digest; the consuming `prompt-projection@1` reference when consumed; retention pins for every source event/object/artifact dependency; command/transaction ID; provenance and taint; and status `requested|completed|consumed|failed|uncertain|expired` with typed failure/uncertainty details. `completed` requires a verified summary artifact; `consumed` additionally requires a committed prompt projection; `failed` cannot be consumed; `uncertain` is never regenerated or consumed automatically; and expiry cannot remove source descriptors still pinned by a consuming projection or gate.

`compaction-record@1` never stores a branch summary. Branch summaries never change the active compaction pointer, and compaction records never stand in for abandoned-branch context. A branch remains within one session/event tree; a fork or clone creates a new session manifest and lineage edge.
...
| `goal-scheduler.mjs` | Caller-admitted execution goals, progress records, schedule/heartbeat continuation leases, claims/delivery/observation/interruption, and fail-closed lifecycle transitions. | `admitExecutionGoal()`, `recordProgress()`, `requestCompletion()`, `admitSchedule()`, `pauseSchedule()`, `resumeSchedule()`, `claimDue()`, `recordDelivery()`, `interrupt()`, `markDeliveryUncertain()`, `cancelSchedule()`, `expireLease()` |
...
- `compaction-record@1`: compaction owner, session and active compaction pointer, exact compacted source range/digests, first retained event, previous compaction ID, summary artifact/provider invocation/projection refs, token facts, dependency pins, validation, attempts, `requested|completed|failed|uncertain|retry-exhausted` status, typed failure state, and command/transaction ID. It contains no branch-summary payload or source/target navigation semantics.
- `branch-summary@1`: source and target session/branch IDs and head event IDs/digests, common-ancestor event ID/digest, exact summarized event ranges, provider invocation and resource-fact refs, immutable summary artifact ref, consuming prompt-projection ref, retention pins, command/transaction ID, provenance/taint, and `requested|completed|consumed|failed|uncertain|expired` status with typed failure details.
...
- `message@1`: host-derived sender, recipient, durable family/topology snapshot, kind (`request`, `result`, `progress`, `correction`, `steer`, `follow-up`), idempotency/dedup key, bounded body and object/evidence refs, attempts and limits, and exactly one state from `enqueued|delivery-attempted|delivered|acknowledged|failed|expired|cancelled|uncertain`; control kinds require an authorized caller role.
- `execution-goal@1`, `continuation-lease@1`, `schedule@1`, and `gate-result-public@1`: caller attachment/admission, objective/progress, finite cumulative budgets, independent gate identity, bounded public category, opaque private-record ID, and terminal reason. `schedule@1` uses schedule state `proposed|active|paused|exhausted|cancelled|lease-expired` plus per-tick delivery state `due|claimed|delivered|observed|interrupted|delivery-uncertain`; `schedule.admitted` performs `proposed -> active`, and the two state namespaces are not interchangeable. The sealed evaluator store separately owns `gate-result-private@1` and any keyed commitment; neither digest nor commitment key is worker-readable.
...
- Context/projection/compaction: `context.registered`, `context.searched`, `context.sliced`, `context.aggregated`, `context.serialized`, `context.materialized`, `context.released`, `context.gc-collected`, `context.tombstoned`, `context.transfer.admitted`, `context.transfer.materialized`, `context.transfer.failed`, `projection.planned`, `projection.committed`, `projection.rejected`, `caller.context.observed`, `compaction.requested`, `compaction.completed`, `compaction.failed`, `compaction.uncertain`, `compaction.retry-exhausted`, `caller.compaction.observed`, `checkpoint.created`, `checkpoint.verified`.
- Branch/fork: `branch.created`, `branch.switched`, `branch.summary.requested`, `branch.summary.completed`, `branch.summary.failed`, `branch.summary.uncertain`, `branch.summary.consumed`, `branch.summary.expired`, `session.forked`, `session.cloned`.
...
- Recursive work/messages: `child.admission.requested`, `child.budget.reserved`, `child.spawned`, `child.state.changed`, `child.result.ready`, `child.retained`, `message.enqueued`, `message.deduplicated`, `message.delivery.attempted`, `message.delivered`, `message.acknowledged`, `message.failed`, `message.expired`, `message.cancelled`, `message.uncertain`.
- Continuation/stops: `execution-goal.admitted`, `execution-goal.progressed`, `execution-goal.completion.requested`, `execution-goal.completed`, `execution-goal.blocked`, `schedule.admitted`, `schedule.paused`, `schedule.resumed`, `schedule.due`, `schedule.claimed`, `schedule.delivered`, `schedule.observed`, `schedule.interrupted`, `schedule.delivery-uncertain`, `schedule.tick.coalesced`, `schedule.exhausted`, `schedule.cancelled`, `schedule.lease-expired`, `gate.started`, `gate.passed`, `gate.failed`, `gate.error`, `stop.approval-needed`, `stop.insufficient-input`, `stop.limit-exhausted`.
...
Every `branch.summary.*`, `message.*`, and `schedule.*` lifecycle event references its exact durable object ID/schema version, previous state, next state, command/transaction ID, root generation, and causal IDs. Branch-summary events additionally reference `branch-summary@1`; message and schedule events reference `message@1` and `schedule@1`. A generic `state.changed` event cannot substitute for one of these declared transitions.
...
- Branch summary: `requested -> completed -> consumed|expired`; `requested -> failed|uncertain`; a completed unconsumed record may expire only after its pins permit it. `uncertain` requires operator disposition and cannot be consumed or regenerated automatically. No transition changes a compaction pointer.
...
- Continuation/schedule: `proposed -> active <-> paused`, with admission performing `proposed -> active`; each tick uses `due -> claimed -> delivered -> observed|interrupted|delivery-uncertain`. Claim atomically advances/coalesces the schedule before delivery. Terminal states are `exhausted|cancelled|lease-expired`; missed ticks are coalesced by policy, never silently replayed. Every transition is represented by the corresponding `schedule.*` event above.
...
Every durable message-state change commits the exactly named event in Section 7.4. `message.cancelled` is idempotent for an already-cancelled message and cannot follow `acknowledged`; `message.uncertain` is required when a crash or transport loss leaves target processing unprovable and cannot transition back to delivery automatically. Recovery may finalize a state only from a durable target acknowledgement or operator disposition linked to the original command ID. Unknown events, unknown states, and illegal pairs such as `expired -> delivered`, `cancelled -> delivery-attempted`, or `uncertain -> delivered` without disposition fail closed and leave the previous committed state intact.
...
- Implement all generic context verbs through `ContextRegistry`/`ProjectionManager`; implement the package-child `CompactionManager` state machine against a synthetic provider/session adapter, caller compaction observation, in-session branches, and cross-session fork/clone lineage. Implement distinct `compaction-record@1` and `branch-summary@1` persistence: a branch summary transaction binds source/target heads, common ancestor, exact ranges, provider/resource facts, verified summary artifact, consuming projection, retention pins, provenance/taint, and status/failure without writing the active compaction pointer. Root/Codex L1 remains caller-owned, and production package-child compaction stays disabled until Phase 3 has durable child sessions.
...
**Invariants:** detach is not cancellation or budget reset; attach does not replay effects; one root generation has one writer; each recovered kernel has a new epoch/token; active traversal is aborted on owner loss; recovery never auto-reruns a live query/provider uncertainty; artifact-backed rematerialization is distinct from source reacquisition; caller/root compaction is never presented as package-owned; `branch-summary@1` and `compaction-record@1` never substitute for or mutate each other; fork/clone is not an in-session branch.

**Acceptance and tests:** protocol negotiation downgrade/denial, stale cursor/generation, competing lease, supervisor restart/adoption, clean/forced shutdown, bounded recovery exhaustion; kill child/root worker/supervisor during idle/eval/traversal/artifact/context/projection/compaction and every compaction pointer boundary; repeat recovery idempotently; exact-event versus descriptor-only versus payload-expired recovery; old handles stale; PEEK stale until rematerialization; module roots confined; no duplicate external/provider effect; operator inspection and busy steer/follow-up/cancel behavior; branch switch/summary and fork/clone lineage. For `branch-summary@1`, crash before/after provider dispatch, provider receipt, summary-artifact commit, record commit, and consuming-projection commit; prove idempotent same-command recovery, uncertain provider non-replay, failure cannot be consumed, retention pins survive restart, an expired unconsumed artifact preserves descriptors, and every unknown/illegal branch-summary transition is denied. Provider/resource usage for child answer, compaction, and branch summary remains separately attributable, and no branch-summary event changes the compaction pointer.
...
**Stop/go:** go only after a 100-cycle synthetic crash/recover soak has zero false completions, duplicate/uncertain effect executions, split-brain commits, context-owner ambiguity, compaction/branch-summary conflation, or authority widening. Stop if safe status cannot distinguish `failed`, `aborted`, `uncertain`, `payload-expired`, and `not-reconstructable`, if a branch-summary status lacks its exact event, or if any operator action exposes new worker authority.
...
- Connect each durable child's provider adapter to `ProjectionManager` and the already-tested `CompactionManager`; pin its `compaction-record@1` and separate `branch-summary@1` settings and charge those provider roles to the same reserved root budget. Do not route caller/root compaction or legacy one-shot `rlm.query()` through this path by implication.
...
- Implement finite `continuation-lease@1` and `schedule@1` records admitted by that caller using the exact Section 7.4 vocabulary. A due tick emits `schedule.due`, is durably claimed, and advances its next due/coalesced state in the same transaction before provider delivery. Delivery, provider acceptance, worker observation, interruption, and delivery uncertainty emit their distinct events. `schedule.delivery-uncertain` is not replayed automatically; the caller/operator must inspect and dispose it.
- Coalesce missed ticks to at most one policy-defined pending continuation carrying the missed interval/count; never replay a backlog as separate prompts. Pause/resume/cancel/lease expiry emit `schedule.paused`, `schedule.resumed`, `schedule.cancelled`, or `schedule.lease-expired`; paused/cancelled/expired schedules cannot claim. Host restart resumes only active, unexpired leases under the same root generation and cumulative budget.
...
**Invariants:** no caller admission means no package continuation; execution goals do not grant capabilities or become parent goals; a schedule claim does not reset cumulative budget; heartbeat prompts cannot modify immutable policy; each schedule/tick state change has its exact event; paused/cancelled/lease-expired schedules cannot claim; claims and `delivery-uncertain` effects are never replayed by inference; completion is requested by a worker and decided by gate plus authorized caller/host; blocked/limit-exhausted/interrupted/insufficient-input is not complete; private scoring, its digest, and commitment key never enter worker-visible L1/L2/L3-public.

**Acceptance and tests:** unadmitted goal/schedule is rejected; caller-attached execution record survives restart without changing caller status; concurrent workers cannot double-claim; same command/idempotency key for pause/resume/cancel/expiry/claim returns the original transition and a different digest conflicts. Inject crashes before/after `schedule.paused`, `schedule.cancelled`, `schedule.lease-expired`, claim, provider delivery, `schedule.delivery-uncertain`, observation, and interruption commits; recovery preserves the last committed state, never replays an uncertain delivery, and never claims a paused/cancelled/expired schedule. Late ticks coalesce exactly per policy. Reject unknown schedule events/states and illegal transitions such as `cancelled -> active`, `lease-expired -> claimed`, `paused -> claimed`, or `delivery-uncertain -> delivered` without explicit disposition. A failed gate returns only an opaque record ID and bounded non-solution category while continuation stays within remaining budget; passing a narrow gate states its scope; approval-needed and insufficient-input paths stop without a live request.
...
**Stop/go:** go only if each due tick has at most one durable claim, every declared state has its exact fail-closed event, pause/cancel/lease expiry survive restart, observation is explicit rather than assumed, missed ticks coalesce, `delivery-uncertain` is not replayed, and continuations cannot expand authority or budget. Stop on any unknown/illegal accepted transition, or if the worker can select/modify its own gate, mark itself complete, or receive private reference details.
...
| D-016 | Branch, fork/clone, compaction, and branch summary are distinct: branch shares a session tree; fork/clone creates a session; `branch-summary@1` is a separate durable schema and never a `compaction-record@1` payload. | Proposed; Prime-fidelity gate | Preserves navigation/recovery semantics without flattening evidence or conflating summary lifecycles. |
...
| D-022 | Messages use host-derived sender, durable family topology, idempotent enqueue/deduplicated processing, bounded attempts/queues, explicit busy/restart behavior, and exact cancelled/uncertain events with fail-closed transitions. | Proposed; required messaging gate | Coordination cannot forge identity, transfer authority, claim exactly-once effects, or recover through an undeclared state. |
...
| D-024 | Package goals are caller-admitted execution records and schedules are continuation leases; exact pause/cancel/lease-expiry/delivery-uncertain events close the fail-closed lifecycle, claims advance before delivery, missed ticks coalesce, and uncertain delivery is not replayed. | Proposed; required compatibility gate | Avoids a competing goal graph, undeclared recovery states, and duplicate autonomous turns. |
...
4. Add model-based tests for the proposed command journal/state machines and crash injection table without implementing production durability, including complete event coverage and unknown-transition denial for `branch-summary@1`, `schedule@1`, and `message@1`.
...
- append-only event history, in-session branches, fork/clone session creation, distinct `compaction-record@1` versus `branch-summary@1` schemas/events/lifecycles, exact original-event recovery, kernel snapshots, retention expiry, and recreation of non-serializable values;
...
- durable family-scoped message queues, host-derived sender, dedup/attempt/failure/queue bounds, explicit cancelled/uncertain events with crash non-replay, busy-child steer/follow-up, retained-child/restart behavior, and bounded observe behavior;
...
- autonomous continuations, caller-admitted execution goals/leases, heartbeats, exact paused/cancelled/lease-expired/delivery-uncertain schedule events, pre-delivery claims, missed-tick coalescing, non-replay of uncertain delivery, retry policy, completion/end-condition gates, and limit-exhaustion/interruption semantics;
